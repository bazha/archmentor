// Creational GoF patterns (beyond the seed Factory Method / Abstract Factory).
...
export const creational: Concept[] = [
  {
    ...
    solution:
      "Класс сам контролирует своё создание: конструктор объявляется приватным, чтобы запретить new извне, а статический метод getInstance() лениво создаёт экземпляр при первом обращении и при всех последующих возвращает один и тот же объект.",
    codeExample: {
      lang: 'typescript',
      code: [
        "class AppConfig {",
    ...
    related: ["factory-method", "abstract-factory", "flyweight", "dip"],
  ...
  },
  {
    ...
    related: ["abstract-factory", "factory-method", "prototype", "composite"],
    tags: ["паттерны", "порождающие"],
  },
  {
    id: "prototype",
    ...
    definition:
      "Задаёт виды создаваемых объектов с помощью прототипического экземпляра и создаёт новые объекты путём копирования этого прототипа.",
    problem:
      "Нужно создавать копии объектов, но клиент не должен зависеть от их конкретных классов. Собрать точную копию снаружи невозможно или дорого: часть состояния скрыта в приватных полях, а инициализация (запросы к БД, сети, тяжёлые вычисления) слишком затратна, чтобы повторять её для каждого экземпляра.",
    solution:
...
export const creationalQuestions: Question[] = [
  ...
  },
  {
    ...
    options: ["Flyweight", "Abstract Factory", "Singleton", "Facade"],
    correctIndex: 2,
    explanation:
  ...
  },
  {
    ...
    options: ["Builder", "Prototype", "Memento", "Factory Method"],
    correctIndex: 1,
    explanation:

...
export const structural: Concept[] = [
  {
    ...
    definition:
      "Преобразует интерфейс класса в другой интерфейс, ожидаемый клиентом. Adapter позволяет совместно работать классам, которые иначе не могли бы этого из-за несовместимых интерфейсов.",
    problem:
      "Есть готовый полезный класс (сторонняя библиотека, легаси-код), но его интерфейс не совпадает с тем, который ожидает клиентский код. Править чужой или уже используемый код нельзя или слишком рискованно, а переписывать клиента под чужой интерфейс — значит привязать его к конкретной реализации.",
    solution:
      "Вводим объект-адаптер, который реализует целевой интерфейс клиента и хранит ссылку на адаптируемый объект (adaptee). Каждый вызов целевого интерфейса адаптер транслирует в вызовы adaptee, попутно преобразуя данные. Клиент работает только с целевым интерфейсом и ничего не знает об адаптируемом классе.",
    codeExample: {
      lang: 'typescript',
      code: [
  ...
  },
  {
    ...
    definition:
      "Отделяет абстракцию от её реализации так, чтобы обе могли изменяться независимо. Вместо наследования абстракция держит ссылку на объект-реализацию и делегирует ему работу.",
    problem:
    ...
    related: ["adapter", "strategy", "abstract-factory", "composition-vs-inheritance"],
    tags: ["паттерны", "структурные"],
  },
  {
    id: "composite",
    ...
    definition:
      "Компонует объекты в древовидные структуры для представления иерархий «часть–целое». Composite позволяет клиентам единообразно трактовать как отдельные объекты, так и их композиции.",
    problem:
      "Клиент работает с рекурсивной структурой (UI-дерево, файловая система, орг-структура), где есть простые элементы и контейнеры из элементов. Без общего интерфейса код клиента забит проверками «лист это или контейнер» и ветвлением на каждый случай, а каждая новая операция дублирует этот разбор.",
    solution:
      "Объявляем общий интерфейс Component с операциями, осмысленными и для листа, и для контейнера. Leaf реализует операцию сам, а Composite хранит список дочерних компонентов и делегирует операцию им, рекурсивно агрегируя результат. Клиент вызывает операцию через интерфейс и не различает лист и поддерево.",
    codeExample: {
      lang: 'typescript',
      code: [
  ...
  },
  {
    ...
    definition:
      "Динамически добавляет объекту новые обязанности, оборачивая его в объект с тем же интерфейсом. Decorator — гибкая альтернатива порождению подклассов для расширения функциональности.",
    problem:
      "Объекту нужно добавлять поведение (логирование, сжатие, шифрование) в разных сочетаниях. Наследование расширяет класс статически, на этапе компиляции, и на каждую комбинацию обязанностей требует свой подкласс — иерархия растёт комбинаторно.",
    solution:
    ...
    related: ["proxy", "adapter", "composite", "composition-vs-inheritance"],
    tags: ["паттерны", "структурные"],
  },
  {
    id: "facade",
    ...
    definition:
      "Предоставляет унифицированный интерфейс к набору интерфейсов подсистемы. Facade определяет интерфейс более высокого уровня, который упрощает использование подсистемы.",
    problem:
      "Для типовой задачи клиенту приходится знать десяток внутренних классов подсистемы, порядок их вызовов и правила инициализации. Клиентский код жёстко связывается с внутренностями подсистемы, и любая её перестройка ломает всех клиентов.",
    solution:
      "Вводим класс-фасад с несколькими высокоуровневыми методами под типовые сценарии. Фасад сам знает, каким объектам подсистемы и в каком порядке делегировать работу; клиент общается только с фасадом, но при необходимости может обращаться к классам подсистемы напрямую.",
    codeExample: {
      lang: 'typescript',
      code: [
    ...
    related: ["adapter", "mediator", "proxy"],
    tags: ["паттерны", "структурные"],
  },
  {
    id: "flyweight",
    name: "Flyweight",
    aka: ["Приспособленец", "Cache"],
    category: "structural",
    grade: "senior",
    ...
    related: ["singleton", "prototype", "composite", "proxy"],
    tags: ["паттерны", "структурные"],
  },
  {
    id: "proxy",
    ...
    definition:
      "Предоставляет суррогат (заместитель) другого объекта для контроля доступа к нему. Proxy реализует тот же интерфейс, что и реальный объект, поэтому для клиента подмена прозрачна.",
    problem:
...
export const structuralQuestions: Question[] = [
  ...
  },
  {
    ...
    options: ["Strategy", "Adapter", "Bridge", "Abstract Factory"],
    correctIndex: 2,
    explanation:
  ...
  },
  {
    ...
    options: ["Decorator", "Composite", "Visitor", "Iterator"],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    options: ["Mediator", "Facade", "Adapter", "Proxy"],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    options: ["Proxy", "Flyweight", "Singleton", "Prototype"],
    correctIndex: 1,
    explanation:

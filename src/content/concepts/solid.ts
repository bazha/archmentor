...
export const solid: Concept[] = [
  ...
  },
  {
    ...
    problem: {
      ru: "Square наследует Rectangle и переопределяет setWidth/setHeight так, что ширина и высота меняются вместе, а не независимо. Клиент, работающий с Rectangle и ожидающий, что setWidth не затронет высоту (постусловие базового типа), получает неверную площадь при подстановке Square — контракт молча нарушен. Тот же класс проблем возникает в паре Circle–Ellipse: эллипс с независимыми полуосями не может честно заменить окружность с единственным радиусом. Типичный симптом такого нарушения в реальном коде — появление instanceof/type-check в клиентском коде («если это Square, обработать иначе»): клиент вынужден вручную обходить нарушенный контракт вместо того, чтобы доверять полиморфизму.",
      en: "Square inherits from Rectangle and overrides setWidth/setHeight so that width and height always change together instead of independently. A client that works with a Rectangle and expects setWidth not to affect the height (the base type's postcondition) gets the wrong area when a Square is substituted in — the contract is silently broken. The same class of problem shows up in the classic Circle–Ellipse pair: an ellipse with independent semi-axes can't honestly substitute for a circle with a single radius. A typical real-world symptom of this violation is an instanceof/type-check creeping into client code (\"if it's a Square, handle it differently\"): the client is forced to manually work around the broken contract instead of trusting polymorphism.",
    },
    solution: {

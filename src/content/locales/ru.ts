...
export const conceptProse: Record<string, ConceptProse> = {
  ...
  "cap-theorem": {
    "tagline": "При сетевой партиции распределённое хранилище вынуждено выбирать между согласованностью и доступностью",
    "definition": "Теорема CAP (Эрик Брюэр, формализована Гилбертом и Линчем) утверждает, что распределённое хранилище данных не может одновременно гарантировать все три свойства: Consistency (каждое чтение видит последнюю запись), Availability (каждый запрос получает ответ без ошибки) и Partition tolerance (система продолжает работать при потере или задержке сообщений между узлами). Поскольку в реальных сетях партиции случаются, отказаться от устойчивости к партициям нельзя, поэтому фактический, непрерывный выбор системы — это выбор между согласованностью (CP) и доступностью (AP) на время партиции.",
    "problem": "Распределённые системы реплицируют данные между узлами и дата-центрами. Когда сеть разделяется, изолированная группа узлов больше не может связаться с лидером или остальными репликами, но продолжает получать запросы клиентов. Инженеру нужно явно решить, что делают эти узлы: продолжать отвечать данными, которые могут быть устаревшими или противоречивыми (доступность), или отказывать в ответе, пока партиция не исчезнет и кворум снова не станет достижим (согласованность). Оставить это решение неявным — получить один из худших исходов: бесконечное зависание или неверный ответ, который никто сознательно не выбирал.",
    "solution": "Явно заложить это решение в код клиента или сервиса: выбрать CP (блокировать или отклонять запросы на стороне меньшинства, пока кворум недостижим) или AP (отдавать локальные данные с пометкой о возможной устарелости) — и написать оба пути в коде, а не полагаться на happy path. В примере ниже клиент воспринимает таймаут основного узла как признак партиции и намеренно деградирует до чтения возможно устаревшей реплики, помечая ответ соответствующим образом, вместо зависания или падения.",
    "code": "// Клиент деградирует до чтения возможно устаревших данных вместо зависания\n// или падения, когда основной узел недоступен из-за сетевой партиции (выбор AP).\ninterface Reading { value: number; asOf: number; stale: boolean; }\n\nclass PartitionAwareClient {\n  constructor(\n    private primary: { read(): Promise<Reading> },\n    private replica: { read(): Promise<Reading> },\n    private timeoutMs = 300,\n  ) {}\n\n  async read(): Promise<Reading> {\n    try {\n      return await this.withTimeout(this.primary.read(), this.timeoutMs);\n    } catch {\n      // Основной узел недоступен — это и есть партиция. Выбираем доступность:\n      // отдаём последнее известное значение реплики вместо блокировки вызывающего.\n      const r = await this.replica.read();\n      return { ...r, stale: true };\n    }\n  }\n\n  private withTimeout<T>(p: Promise<T>, ms: number): Promise<T> {\n    return Promise.race([\n      p,\n      new Promise<T>((_, reject) => setTimeout(() => reject(new Error('timeout')), ms)),\n    ]);\n  }\n}",
    "pros": [
      "Заставляет явно, письменно зафиксировать поведение при партиции — вместо случайного зависания или тихого неверного ответа",
      "Делает компромисс между доступностью и согласованностью видимым для вызывающей стороны через явный признак вроде `stale`, чтобы клиент сам решал, как реагировать",
      "Позволяет разным операциям одной системы выбирать разные стороны — строгие чтения для проверки баланса, допускающие устарелость чтения для каталога товаров"
    ],
    "cons": [
      "CAP описывает только поведение во время партиции — вне партиции теорема ничего не говорит о задержках, для этого полезнее модель PACELC",
      "Буква C в CAP — это линеаризуемость, гарантия намного строже того, что обычно понимают под «согласованностью»; их смешение создаёт ложную уверенность",
      "Сводит богатое пространство моделей согласованности к бинарному выбору C/A, упрощая системы с настраиваемой согласованностью на уровне запроса"
    ],
    "tradeoffs": [
      "Согласованность (отклонять или блокировать запросы, пока кворум недостижим) против доступности (отдавать локальные, возможно устаревшие данные) — во время партиции нельзя получить и то, и другое",
      "Выбор CP меняет время безотказной работы на корректность: клиенты на стороне меньшинства получают ошибки, а не неверные ответы",
      "Выбор AP меняет корректность на время безотказной работы: клиенты получают ответы по обе стороны партиции, возможно противоречивые, которые придётся согласовать позже"
    ],
    "whenToUse": [
      "Проектирование поведения распределённого хранилища или сервиса при недоступности узла или дата-центра",
      "Выбор между базами данных или настройками согласованности под конкретную нагрузку — финансовые реестры склоняются к CP, данные сессий или присутствия — к AP",
      "Объяснение стейкхолдерам, почему мультирегиональная система не может обещать одновременно нулевой простой и всегда свежие данные"
    ],
    "whenNotToUse": [
      "У однонодовой системы или системы без сетевых партиций между репликами нет предмета выбора — CAP неприменима",
      "Для рассуждений об устоявшихся задержках и пропускной способности используйте PACELC или обычный анализ latency/очередей"
    ]
  },
  "consistency-models": {
    "tagline": "Точное название того, насколько устаревшим или переставленным может быть чтение — от линеаризуемости до eventual",
    "definition": "Модель согласованности — это контракт хранилища о том, какие значения может вернуть чтение относительно истории записей: от линеаризуемости (каждая операция как будто происходит мгновенно, в едином глобальном порядке) через каузальную согласованность и гарантии уровня сессии — read-your-writes, monotonic reads — до eventual consistency (реплики сходятся к одному значению, если новых записей больше нет, но без гарантии срока). Чем сильнее модель, тем ближе поведение чтений к единственной, нераспределённой копии данных; чем слабее модель, тем больше устарелости или переупорядочивания должен терпеть вызывающий код.",
    "problem": "У распределённого хранилища с несколькими репликами нет единого момента, в который определено «то самое» значение ключа — разные реплики законно могут хранить разные версии в один и тот же момент по часам. Без точного названия гарантии баг-репорты сводятся к расплывчатому «иногда всё устарело», код начинает предполагать более сильные гарантии, чем хранилище реально даёт, а слово «согласованность» у разных команд означает совершенно разное поведение.",
    "solution": "Выбрать и задокументировать самую слабую модель согласованности, которая реально нужна конкретной фиче, и явно обеспечить именно её, а не надеяться, что настройки хранилища по умолчанию окажутся достаточно сильными. Пример ниже реализует read-your-writes: клиент запоминает наибольшую версию своей последней записи (токен сессии) и читает только с реплики, догнавшей хотя бы эту версию, поэтому клиент никогда не увидит, как его собственная запись «исчезла», хотя хранилище в целом лишь eventually consistent.",
    "code": "// Read-your-writes: клиент запоминает наибольшую версию своей последней\n// записи («токен сессии»), и реплика должна догнать эту версию, прежде\n// чем обслуживать чтения этого клиента — иначе клиент увидел бы, как его\n// собственная запись «исчезла».\ninterface Replica { version(): number; read(key: string): string | undefined; }\n\nclass SessionConsistentReader {\n  private lastWrittenVersion = 0;\n\n  recordWrite(version: number): void {\n    this.lastWrittenVersion = Math.max(this.lastWrittenVersion, version);\n  }\n\n  read(key: string, replicas: Replica[]): string | undefined {\n    // Направляем чтение на любую реплику, догнавшую (как минимум) последнюю\n    // запись этой сессии.\n    const caughtUp = replicas.find((r) => r.version() >= this.lastWrittenVersion);\n    if (!caughtUp) throw new Error('ни одна реплика ещё не догнала — повторите позже или читайте с primary');\n    return caughtUp.read(key);\n  }\n}",
    "pros": [
      "Точное название модели (линеаризуемая, каузальная, read-your-writes, eventual) превращает расплывчатый отчёт «иногда всё устарело» в конкретную, проверяемую гарантию",
      "Более слабые модели обычно дешевле — ниже задержка, выше доступность, — поэтому выбор самой слабой модели, реально нужной фиче, экономит реальные ресурсы",
      "Гарантии уровня сессии вроде read-your-writes устраняют самый заметный для пользователя симптом eventual consistency за малую долю стоимости полной линеаризуемости"
    ],
    "cons": [
      "Слабые модели перекладывают сложность на каждый клиентский код, читающий данные: он должен терпеть и согласовывать устаревшие или переставленные значения",
      "Гарантии плохо компонуются: read-your-writes в одном сервисе ничего не говорит о том, что увидит второй сервис при чтении тех же данных",
      "Сама терминология — ловушка: «eventual consistency» скрывает, насколько большим может быть окно устарелости и насколько неожиданными — аномалии под реальной нагрузкой"
    ],
    "tradeoffs": [
      "Более сильные модели упрощают рассуждения для каждого вызывающего кода, но стоят задержки и доступности, поскольку обычно требуют координации на каждой операции",
      "Более слабые модели быстры и доступны, но переносят работу по обеспечению корректности на прикладной код — разрешение конфликтов, токены сессии, идемпотентность",
      "Гарантии уровня сессии дёшево закрывают частый случай, но они локальны для клиента, а не глобальны — два разных пользователя всё ещё могут увидеть разный порядок"
    ],
    "whenToUse": [
      "Точное описание контракта API хранилища, чтобы клиентский код знал, какую именно устарелость он обязан терпеть",
      "Разбор отчётов вида «моя собственная запись исчезла» или «значения пошли назад» — обычно это отсутствующая гарантия сессии, а не баг в хранилище",
      "Решение для каждой фичи отдельно: действительно ли нужны линеаризуемые чтения (баланс счёта) или достаточно eventual consistency (счётчик лайков)"
    ],
    "whenNotToUse": [
      "У однонодовой системы без репликации есть единственный естественный порядок и нет модели согласованности для выбора",
      "Преждевременное усиление — навешивание токенов сессии или кворумных чтений на фичу, где ни разу не проявился баг устарелости, добавляет стоимость без пользы для пользователя"
    ]
  },
  "quorum": {
    "tagline": "Пересекающиеся наборы чтения и записи (R + W > N), гарантирующие, что чтение увидит последнюю подтверждённую запись",
    "definition": "В реплицированном хранилище без выделенного лидера, с N репликами, кворумная схема требует подтверждения записи от W реплик и сбора ответов на чтение от R реплик, причём W и R выбираются так, чтобы R + W > N. Это неравенство гарантирует, что набор узлов, затронутых любым чтением, пересекается с набором узлов любой предыдущей записи хотя бы в одном узле — поэтому чтение математически гарантированно увидит самое свежее подтверждённое значение (после разрешения конфликтующих версий), даже если не все реплики актуальны.",
    "problem": "Без лидера, сериализующего каждую операцию, ничто автоматически не мешает чтению попасть только на реплики, ещё не увидевшие последнюю запись. Требование подтверждения от всех N реплик (W=N) делает систему недоступной в момент, когда любой единственный узел медленный или недоступен; требование подтверждения только от одной (W=1) делает записи быстрыми, но не даёт гарантии, что более позднее чтение их увидит. Ни одна из крайностей не подходит системе, которая должна оставаться одновременно быстрой и достаточно корректной.",
    "solution": "Выбирать W и R для каждой операции так, чтобы R + W > N, гарантируя пересечение любого набора чтения с любым набором записи, и использовать read repair, чтобы возвращать самое свежее значение на реплики, ответившие устаревшим. Пример ниже реализует именно проверку R + W > N плюс read repair: при каждом чтении сравниваются версии у R опрошенных реплик, возвращается самая новая, а отставшие реплики дозаписываются этим значением.",
    "code": "// Кворумные чтение и запись: если R + W > N, любой набор чтения и любой\n// набор записи пересекаются хотя бы в одном узле, поэтому чтение\n// гарантированно увидит последнюю подтверждённую запись.\ninterface Node { id: string; write(key: string, value: string, version: number): void; read(key: string): { value: string; version: number } | undefined; }\n\nclass QuorumStore {\n  constructor(private nodes: Node[], private w: number, private r: number) {\n    if (w + r <= nodes.length) throw new Error('кворум настроен неверно: R + W должно быть больше N');\n  }\n\n  write(key: string, value: string, version: number): void {\n    const acked = this.nodes.slice(0, this.w); // на практике: первые W, подтвердившие запись\n    acked.forEach((n) => n.write(key, value, version));\n  }\n\n  read(key: string): string | undefined {\n    const responses = this.nodes.slice(0, this.r).map((n) => ({ node: n, res: n.read(key) }));\n    const latest = responses.reduce((best, cur) =>\n      (cur.res?.version ?? -1) > (best.res?.version ?? -1) ? cur : best);\n\n    // Read repair: узлы, ответившие устаревшей (или отсутствующей) версией,\n    // получают актуальное значение, поэтому следующему кворумному чтению\n    // нужно меньше «ремонта».\n    if (latest.res) {\n      responses\n        .filter((r) => (r.res?.version ?? -1) < latest.res!.version)\n        .forEach((r) => r.node.write(key, latest.res!.value, latest.res!.version));\n    }\n    return latest.res?.value;\n  }\n}",
    "pros": [
      "Настраивается для каждой операции: повысить W для критичных к безопасности записей, повысить R для чувствительных к устарелости чтений — без изменения топологии репликации",
      "Устойчиво к отказам узлов — пока достижим кворум из N узлов, чтения и записи продолжают работать",
      "Read repair работает попутно с обычными чтениями, поэтому устаревшие реплики самовосстанавливаются без отдельной фоновой задачи"
    ],
    "cons": [
      "R + W > N гарантирует пересечение, а не порядок во времени конкурентных записей — конфликтующие версии всё ещё возможны и требуют правила разрешения (last-write-wins, vector clocks)",
      "Каждое чтение и запись затрагивают несколько узлов, поэтому задержка ограничена самым медленным из R или W ответивших, а не самым быстрым одиночным узлом",
      "Выбор W=N или R=N «для дополнительной надёжности» незаметно превращает хранилище в CP — отказ любого единственного узла тогда блокирует операцию"
    ],
    "tradeoffs": [
      "Более высокий W означает более безопасные, но более медленные записи и меньшую доступность записи при отказах узлов",
      "Более высокий R означает более свежие, но более медленные чтения; низкий R быстр, но может вернуть значение, до которого read repair ещё не добрался",
      "R + W > N даёт гарантию read-after-write дешевле, чем маршрутизация всего через единственного лидера, но конкурентные конфликтующие записи при W < N всё равно требуют правила слияния"
    ],
    "whenToUse": [
      "Реплицированное хранилище без лидера (в стиле Dynamo), где согласованность должна настраиваться для каждой операции, а не быть одной фиксированной гарантией",
      "Балансировка нагрузки на чтение и запись и задержки за счёт сдвига соотношения R/W при сохранении инварианта R + W > N",
      "Объяснение, почему хранилище с W=1 может потерять подтверждённую запись, если узел откажет раньше, чем репликация её догонит"
    ],
    "whenNotToUse": [
      "Система с единственным лидером уже сериализует все записи через один узел — кворумная арифметика применяется иначе",
      "Нагрузкам, которым нужна настоящая линеаризуемость по всем репликам, лучше подходит лидер на основе консенсуса, а не настраиваемые кворумы"
    ]
  },
  "partitioning": {
    "tagline": "Разбиение набора данных по узлам по ключу, чтобы хранение и пропускная способность записи масштабировались за пределы одной машины",
    "definition": "Партиционирование (шардирование) делит набор данных на непересекающиеся части — шарды, каждая из которых хранится на своём узле, а схема партиционирования отображает ключ на владеющий им шард. Наивная схема вроде `id % N` тривиальна в реализации, но привязывает расположение каждого ключа к общему числу узлов; consistent hashing размещает узлы и ключи на одном хеш-кольце, поэтому добавление или удаление узла переназначает только ближайшие к нему ключи, не трогая остальное отображение.",
    "problem": "У одного узла конечны и хранилище, и пропускная способность записи, и рано или поздно один из ресурсов (или оба) становится узким местом, которое вертикальное масштабирование уже не может обогнать. Разбиение данных по узлам решает проблему объёма, но наивный способ сделать это — `id % N` — пересчитывает владельца почти каждого ключа при изменении N, превращая рутинное изменение ёмкости (добавление ещё одного узла) в полное перемешивание всего набора данных, конкурирующее с живым трафиком.",
    "solution": "Выбрать ключ партиционирования, равномерно распределяющий нагрузку и держащий часто запрашиваемые вместе данные на одном шарде, и использовать consistent hashing вместо остатка от деления, чтобы изменение размера кластера перемещало только те ключи, которые действительно должны переехать. Пример ниже сравнивает оба подхода: `id % N`, где изменение N переназначает почти всё, против хеш-кольца, где добавление узла отбирает ключи только у ближайшего соседа по кольцу.",
    "code": "// Наивный вариант: id % N. Добавление или удаление узла меняет N, из-за\n// чего переназначается почти каждый ключ — полное перемешивание данных.\nfunction shardByModulo(id: number, nodeCount: number): number {\n  return id % nodeCount;\n}\n\n// Consistent hashing: узлы и ключи размещаются на одном кольце хешей;\n// ключ принадлежит первому узлу по часовой стрелке от себя. Добавление\n// узла «отбирает» ключи только у ближайшего соседа по кольцу — остальные\n// остаются на месте.\nclass ConsistentHashRing {\n  private ring: { hash: number; node: string }[] = [];\n\n  addNode(node: string, points = 3): void {\n    for (let i = 0; i < points; i++) {\n      this.ring.push({ hash: this.hash(`${node}#${i}`), node });\n    }\n    this.ring.sort((a, b) => a.hash - b.hash);\n  }\n\n  shardFor(key: string): string {\n    const h = this.hash(key);\n    const point = this.ring.find((p) => p.hash >= h) ?? this.ring[0];\n    return point.node;\n  }\n\n  private hash(s: string): number {\n    let h = 0;\n    for (const ch of s) h = (h * 31 + ch.charCodeAt(0)) >>> 0;\n    return h;\n  }\n}\n\n// shardByModulo(id, 3) против shardByModulo(id, 4): почти все id меняют шард.\n// ring.addNode('node-4'): переезжают только ключи между node-4 и его\n// соседом по часовой стрелке — данные всех остальных остаются на месте.",
    "pros": [
      "Распределяет данные и нагрузку по многим узлам, поэтому и объём хранения, и пропускная способность записи растут примерно линейно с числом узлов",
      "Consistent hashing ограничивает перемещение данных при добавлении или удалении узла примерно 1/N пространства ключей вместо полного перемешивания",
      "Удачно выбранный ключ держит связанные данные на одном шарде, поэтому частые запросы остаются однодардовыми и быстрыми"
    ],
    "cons": [
      "Запросы, затрагивающие несколько шардов — джойны, кросс-шардовые транзакции, глобальная сортировка или агрегация, — превращаются в дорогие fan-out операции",
      "Перекошенный ключ, например один популярный tenant или знаменитость, создаёт горячий шард, который не лечится добавлением новых узлов",
      "Решардинг, даже дешёвый — на основе consistent hashing, — операционно рискован: он перемещает живые данные, пока система продолжает обслуживать трафик"
    ],
    "tradeoffs": [
      "`id % N` проще для понимания, но переназначает почти все ключи при любом изменении топологии; consistent hashing добавляет структуру-кольцо, но делает решардинг локальным",
      "Ключ, равномерно распределяющий нагрузку, избегает горячих шардов, но разбрасывает связанные записи, из-за чего кросс-шардовые запросы становятся обычным случаем, а не исключением",
      "Больше виртуальных узлов на физический узел сглаживает распределение нагрузки, но добавляет учётные расходы и метаданные"
    ],
    "whenToUse": [
      "Хранилище или пропускная способность записи одного узла — узкое место, а вертикальное масштабирование исчерпало запас",
      "Нагрузка естественно делится по стабильному ключу — id тенанта, пользователя, заказа — с небольшим числом кросс-ключевых запросов",
      "Ожидается, что размер кластера будет меняться со временем — это довод в пользу consistent hashing вместо остатка от деления с самого начала"
    ],
    "whenNotToUse": [
      "Набор данных и нагрузка комфортно помещаются на один хорошо реплицированный узел — партиционирование лишь добавляет кросс-шардовую сложность без выгоды",
      "Преобладающие запросы — это кросс-сущностные джойны или агрегации, не совпадающие ни с одним ключом партиционирования"
    ]
  },
  "replication": {
    "tagline": "Хранение копий данных на нескольких узлах ради масштабирования чтения и отказоустойчивости — ценой лага реплик",
    "definition": "Репликация поддерживает несколько копий одних и тех же данных на разных узлах, обычно с одним лидером, принимающим записи, и одним или несколькими последователями (followers), применяющими те же изменения — синхронно (запись считается успешной только после подтверждения последователями) или асинхронно (лидер подтверждает сразу, а последователи догоняют позже). Последователи могут обслуживать чтения и могут быть повышены до лидера при его отказе, ценой того, что асинхронно реплицируемый последователь всегда немного отстаёт от лидера — это его лаг репликации.",
    "problem": "Один узел может обслужить лишь ограниченное число чтений, а его отказ уносит с собой весь набор данных. Простое направление большего трафика чтения на единственный узел не масштабируется за пределы его аппаратных возможностей, а без второй копии данных где-либо сбой диска или падение узла невосстановимы. Наивное чтение «с какой-то копии» данных без учёта того, насколько она может отставать, даёт молча неверные ответы под нагрузкой — как раз тогда, когда дополнительная пропускная способность чтения нужнее всего.",
    "solution": "Реплицировать данные на последователей и направлять на них чтения, чтобы масштабировать пропускную способность чтения и переживать потерю узла, но отслеживать лаг репликации каждого последователя и обслуживать чтениями только те запросы, которые могут его стерпеть — переключаясь на лидера, если ни один последователь недостаточно свеж. Пример ниже направляет чтение на реплику с наименьшим лагом в пределах заданного бюджета, а при отставании всех реплик читает напрямую с лидера.",
    "code": "// Направляем чтение на реплику, только если её отставание не превышает\n// допустимый бюджет — иначе читаем с leader, чтобы получить свежие данные.\ninterface Replica { lagMs(): number; read(key: string): string | undefined; }\ninterface Leader { read(key: string): string | undefined; }\n\nclass LagAwareReadRouter {\n  constructor(\n    private leader: Leader,\n    private replicas: Replica[],\n    private maxLagMs = 1000,\n  ) {}\n\n  read(key: string): string | undefined {\n    const fresh = this.replicas\n      .filter((r) => r.lagMs() <= this.maxLagMs)\n      .sort((a, b) => a.lagMs() - b.lagMs());\n\n    if (fresh.length === 0) {\n      // Все реплики отстают слишком сильно — читаем из источника истины.\n      return this.leader.read(key);\n    }\n    return fresh[0].read(key);\n  }\n}",
    "pros": [
      "Реплики для чтения забирают на себя трафик чтения, разгружая лидера и масштабируя пропускную способность чтения независимо от записи",
      "Последователь может занять место лидера при его отказе, обеспечивая доступность системы при потере узла",
      "Географически распределённые реплики размещают копию данных ближе к удалённым пользователям, снижая задержку чтения"
    ],
    "cons": [
      "При асинхронной репликации последователи отстают, поэтому чтение, направленное на отставшую реплику, может вернуть устаревшие данные",
      "Failover не мгновенен и не бесплатен: обнаружение упавшего лидера и повышение последователя занимает время и может ненадолго потерять нереплицированные записи",
      "Больше реплик — больше fan-out при записи и больше узлов, за лагом, местом на диске и расхождениями которых нужно следить"
    ],
    "tradeoffs": [
      "Синхронная репликация держит последователей свежими, но добавляет задержку записи и может блокировать запись, если последователь медленный или недоступен; асинхронная репликация быстра, но рискует устаревшими чтениями и потерей записей при failover",
      "Направление чтений на ближайшую или наименее загруженную реплику улучшает задержку и пропускную способность ценой гарантий согласованности этих чтений",
      "Больше реплик повышают пропускную способность чтения и отказоустойчивость, но умножают объём записи (write amplification) и операционную поверхность"
    ],
    "whenToUse": [
      "Нагрузка на чтение намного выше нагрузки на запись, и один узел не справляется со всем чтением",
      "Система должна переживать потерю узла без потери доступности",
      "Пользователи географически распределены, и задержка чтения из одного региона неприемлема"
    ],
    "whenNotToUse": [
      "Нагрузка в основном на запись, а чтения редки — реплики в основном лишь добавляют fan-out записи почти без выгоды",
      "Фиче нужно гарантированно свежее чтение при каждом запросе без допуска на лаг — читайте напрямую с лидера"
    ]
  },
  "consensus": {
    "tagline": "Заставить большинство ненадёжных узлов безопасно согласиться на один факт, даже если часть из них отказывает",
    "definition": "Протоколы консенсуса (Paxos, Raft и их производные) позволяют группе узлов согласовать единое значение или последовательность решений — например, кто сейчас лидер или какая запись лога должна быть закоммичена следующей — несмотря на отказы узлов, перезапуски и задержки сообщений, пока достижимо большинство группы. Системы вроде etcd и ZooKeeper предоставляют консенсус как сервис блокировок или координации, обычно используемый для выбора лидера и выдачи fencing token — числа, которое только растёт, поэтому любых двух держателей аренды можно строго упорядочить.",
    "problem": "Распределённая блокировка, построенная на единственном источнике истины, хрупка: процесс может считать, что держит блокировку и действует как лидер, значительно позже фактического истечения аренды — например, из-за долгой паузы сборщика мусора или сетевой задержки. Когда он наконец возобновляет работу и пишет данные, ничто не мешает ему затереть то, что уже записал полноправный новый лидер, избранный тем временем, потому что сама по себе блокировка не несёт доказательства того, кто действительно актуален.",
    "solution": "Использовать сервис блокировок на основе консенсуса, выдающий с каждой арендой монотонно растущий fencing token, и заставлять каждую запись нести этот токен, чтобы хранилище отклоняло любую запись с токеном старше последнего уже принятого. Это превращает «я считаю, что держу блокировку» в значение, которое хранилище может дёшево проверить, закрывая класс багов с «зависшим» лидером, который обычная блокировка на взаимном исключении закрыть не может.",
    "code": "// Сервис блокировок на основе консенсуса (Raft/Paxos под капотом, например\n// etcd, ZooKeeper) выдаёт с каждой арендой монотонно растущий fencing token.\n// Хранилище отклоняет любую запись с токеном старше последнего увиденного —\n// поэтому «зависший» лидер, очнувшийся позже, не может испортить данные,\n// даже если всё ещё считает, что держит блокировку.\ninterface LockService { acquire(name: string): { token: number }; }\ninterface GuardedStorage { write(token: number, value: string): void; }\n\nclass FencedWriter {\n  constructor(private lock: LockService, private storage: GuardedStorage) {}\n\n  writeAsLeader(value: string): void {\n    const { token } = this.lock.acquire('leader-election'); // токен от консенсуса\n    // Даже если процесс «застрянет» здесь из-за долгой паузы GC или сети,\n    // а новый лидер будет избран с большим токеном, эта запись в итоге\n    // будет отклонена хранилищем — и никогда не применится тихо.\n    this.storage.write(token, value);\n  }\n}\n\nclass InMemoryGuardedStorage implements GuardedStorage {\n  private lastToken = 0;\n  write(token: number, value: string): void {\n    if (token < this.lastToken) throw new Error(`устаревшая запись отклонена: токен ${token} < ${this.lastToken}`);\n    this.lastToken = token;\n    // ...сохранить значение...\n  }\n}",
    "pros": [
      "Даёт распределённой системе единый согласованный факт — кто лидер, какое значение закоммичено, — даже когда ни один узел не может полностью доверять ни своим часам, ни одной лишь блокировке взаимного исключения",
      "Fencing token превращает «я считаю, что держу блокировку» в проверяемое, монотонно растущее доказательство, которое хранилище проверяет одним сравнением",
      "Протоколы консенсуса переживают отказ меньшинства узлов, не теряя согласия, в отличие от единственного координатора"
    ],
    "cons": [
      "Каждое согласованное решение требует раунда обмена с большинством узлов, что добавляет задержку по сравнению с чисто локальным решением",
      "Группе консенсуса для прогресса нужно большинство, поэтому она останавливается при его потере, даже если часть узлов меньшинства исправна",
      "Корректные реализации Raft или Paxos печально известны своей тонкостью; написание собственной реализации — частый источник продовых багов с потерей данных"
    ],
    "tradeoffs": [
      "Консенсус даёт единый, глобально согласованный факт ценой раунда с большинством на каждое решение — это намного дороже нескоординированной записи",
      "Fencing token добавляет одно сравнение целых чисел к каждой записи взамен на закрытие класса багов с «зависшим» лидером, которые обычная блокировка закрыть не может",
      "Опора на внешний сервис консенсуса вроде etcd или ZooKeeper снимает сложную часть работы, но делает доступность этого сервиса зависимостью для каждого выбора лидера"
    ],
    "whenToUse": [
      "Выбор единственного лидера или координатора в распределённой системе — кто получает право писать, кто владеет партицией",
      "Любой случай, когда блокировка или аренда дают эксклюзивный доступ к ресурсу, который «зависший» или приостановленный GC держатель мог бы испортить",
      "Коммит значения, с которым должен согласиться каждый узел перед продолжением, например изменение конфигурации кластера"
    ],
    "whenNotToUse": [
      "Единственному процессу или системе с одним очевидным источником истины не нужно согласие между несколькими узлами",
      "Высокопроизводительные операции с низкой задержкой, допускающие eventual consistency, не стоит пропускать через раунд консенсуса на каждую запись — оставьте его для редких координационных решений"
    ]
  },
  "idempotency": {
    "tagline": "Повторный вызов одной и той же операции должен давать тот же эффект, что и один вызов.",
    "definition": "Идемпотентная операция даёт один и тот же результат независимо от того, сколько раз её применили. Для сетевого API это означает, что клиент может безопасно повторить запрос после таймаута или обрыва соединения, не рискуя тем, что реальный побочный эффект — списание с карты, заказ, отправка письма — произойдёт дважды. Обычная реализация — ключ идемпотентности: уникальное значение, которое клиент генерирует один раз на логическую операцию и повторно отправляет при каждой попытке, плюс хранилище на стороне сервера, которое помнит для каждого ключа, выполнялась ли уже операция и какой ответ она дала.",
    "problem": "Сеть ломается в самый неудобный момент: сервер уже обработал запрос, но его ответ не дошёл до клиента. С точки зрения клиента таймаут неотличим от «сервер вообще не получил запрос» — единственное, что выглядит безопасным, — это повторить попытку. Но если первая попытка на самом деле завершилась успешно, наивный повторный запрос выполнит тот же побочный эффект ещё раз: повторное списание с той же карты, дублирующий заказ по той же корзине, вторая копия того же письма. Клиент не может отличить эти две ситуации сам по себе — запоминать, что уже произошло, обязан сервер.",
    "solution": "Клиент генерирует ключ идемпотентности один раз на логическую операцию — а не на каждую HTTP-попытку — и прикладывает его к каждому повтору этой операции. Прежде чем выполнять какую-либо работу, сервер проверяет хранилище по этому ключу: если ключ уже встречался, он возвращает ранее сохранённый результат, не повторяя побочный эффект; если нет — выполняет операцию и сохраняет результат под этим ключом. Побочный эффект и запись о том, что он выполнен, должны записываться вместе, в одной локальной транзакции, чтобы падение между этими двумя шагами не оставило эффект без записанного ключа или ключ без реально закоммиченного эффекта.",
    "code": "// Хранилище ключей идемпотентности: дедуплицирует побочный эффект при повторе,\n// а не сам запрос.\ninterface StoredResult { status: number; body: unknown; }\n\nclass IdempotencyStore {\n  private results = new Map<string, StoredResult>();\n\n  async run(key: string, fn: () => Promise<StoredResult>): Promise<StoredResult> {\n    const cached = this.results.get(key);\n    if (cached) return cached; // повтор с тем же ключом — пропускаем повторное выполнение\n\n    const result = await fn();\n    this.results.set(key, result);\n    return result;\n  }\n}\n\nasync function chargeCard(store: IdempotencyStore, key: string, amountCents: number) {\n  return store.run(key, async () => {\n    const charge = await paymentGateway.charge(amountCents);\n    return { status: 200, body: charge };\n  });\n}",
    "pros": [
      "Повторы становятся безопасными по построению — клиенты и сетевые библиотеки могут агрессивно ретраить таймауты, не рискуя повторным списанием, заказом или сообщением.",
      "Снимает бремя корректности с транспортного уровня — не нужно, чтобы сеть или брокер гарантировали доставку ровно один раз, достаточно доставки хотя бы один раз плюс ключа.",
      "Естественно сочетается с доставкой хотя бы один раз — обработчик вебхука или потребитель очереди становится безопасным для повторной доставки без дополнительной механики."
    ],
    "cons": [
      "Требует собственного хранилища (таблицы или кэша ключей) с политикой хранения/TTL — хранилище без истечения растёт бесконечно, а слишком короткий TTL перестаёт дедуплицировать после истечения окна.",
      "Ключ обязан относиться ровно к одной логической операции; повторное использование ключа для другого payload нужно явно трактовать как конфликт, иначе два разных запроса молча схлопнутся в один.",
      "Сам по себе не делает операцию идемпотентной — побочный эффект всё равно должен быть защищён тем же хранилищем, иначе повтор, обогнавший проверку до завершения первой попытки, всё ещё может продублировать работу."
    ],
    "tradeoffs": [
      "Хранить полное тело ответа (повтор воспроизводит byte-in-byte то, что клиент получил изначально) против хранения только статуса/маркера (дешевле, но повтор не может воспроизвести исходный payload).",
      "Короткий TTL ключа (меньше хранилища, но очень поздний повтор перестаёт дедуплицироваться и выполняется заново) против длинного TTL (надёжнее против поздних повторов, но хранилище занято дольше).",
      "Блокировка ключа на время выполнения первой попытки (конкурентный повтор получает быстрый явный конфликт) против того, чтобы дать конкурентным повторам выполняться параллельно (проще, но требует, чтобы сама операция переносила короткое окно двойного выполнения)."
    ],
    "whenToUse": [
      "Любой внешне вызываемый мутирующий эндпоинт, доступный по ненадёжной сети, где вызывающая сторона может повторить запрос, — платежи, создание заказа, провижининг ресурсов.",
      "Потребители очередей или вебхуков с доставкой хотя бы один раз, которые по определению могут получить одно и то же сообщение больше одного раза.",
      "Шаги внутри распределённого workflow или саги, которые могут быть повторно вызваны после частичного сбоя или рестарта оркестратора."
    ],
    "whenNotToUse": [
      "Чисто читающие эндпоинты — GET без побочного эффекта уже идемпотентен, дедуплицировать хранилищу ключей нечего.",
      "Операции, уже обёрнутые в единую синхронную транзакцию без доступного вызывающей стороне пути повтора, — дополнительное хранилище там ничего не даёт."
    ]
  },
  "transactional-outbox": {
    "tagline": "Записать бизнес-строку и событие outbox в одной локальной транзакции; отдельный релей публикует его позже.",
    "definition": "Паттерн transactional outbox записывает событие как обычную строку в таблице базы данных внутри той же ACID-транзакции, что и бизнес-изменение, которое это событие описывает. Отдельный процесс — релей — читает неопубликованные строки outbox, публикует их в брокер сообщений и помечает как отправленные. Поскольку бизнес-запись и запись события находятся в одной транзакции, они коммитятся или откатываются вместе — не существует окна, в котором одно произошло, а другое нет.",
    "problem": "Запись в базу данных и отдельная публикация в брокер сообщений — это две независимые операции над двумя независимыми системами. Если процесс падает или брокер недоступен между коммитом в базу и публикацией в брокер — или в обратном порядке — одна сторона рассинхронизируется с другой: заказ сохранён, но никто об этом не узнал, либо событие ушло для заказа, который затем не сохранился. Это проблема двойной записи (dual-write problem), и никакие повторные попытки одной лишь публикации её не исправят, потому что две записи никогда не атомарны друг относительно друга.",
    "solution": "Вместо прямой публикации путь записи вставляет событие как строку в таблицу outbox в той же локальной транзакции, что и бизнес-запись, — один ACID-коммит, поэтому происходит либо всё, либо ничего. Отдельный процесс-релей затем читает ещё не опубликованные строки (опрашивая таблицу либо считывая журнал репликации базы данных так же, как это делает change data capture), публикует каждую в брокер и помечает её опубликованной. Сам шаг «опубликовать, затем пометить» у релея — тоже лишь доставка хотя бы один раз: он может упасть между этими двумя действиями, — поэтому потребителям на другой стороне всё равно нужна дедупликация по id события, что связывает этот паттерн с семантиками доставки и идемпотентностью на стороне потребления.",
    "code": "// Обе записи происходят в одной локальной транзакции БД — атомарно по построению.\nasync function placeOrder(db: Db, order: Order) {\n  await db.transaction(async (tx) => {\n    await tx.insert('orders', order);\n    await tx.insert('outbox', {\n      id: crypto.randomUUID(),\n      aggregateId: order.id,\n      type: 'OrderPlaced',\n      payload: JSON.stringify(order),\n      publishedAt: null,\n    });\n  });\n}\n\n// Релей: отдельный процесс, опрашивает неопубликованные строки и публикует их.\nasync function relayOnce(db: Db, broker: Broker) {\n  const rows = await db.query(\n    'SELECT * FROM outbox WHERE published_at IS NULL ORDER BY id LIMIT 100',\n  );\n  for (const row of rows) {\n    await broker.publish(row.type, row.payload); // может упасть до следующей строки\n    await db.query('UPDATE outbox SET published_at = now() WHERE id = $1', [row.id]);\n  }\n}",
    "pros": [
      "Устраняет проблему двойной записи по построению: событие не может закоммититься без бизнес-изменения, и наоборот, потому что они делят одну транзакцию.",
      "Работает с любым брокером без распределённой транзакции (2PC/XA) между базой и брокером — релей общается только со своей базой и отдельно с брокером.",
      "Таблица outbox одновременно служит долговечным, воспроизводимым журналом всего, что произошло с агрегатом, — полезно для аудита или пересборки проекции с нуля."
    ],
    "cons": [
      "Добавляет компонент-релей, который должен работать непрерывно, находиться под наблюдением и масштабироваться вместе с объёмом записи, — это новая операционная поверхность, а не бесплатное дополнение.",
      "Шаг публикации самого релея — тоже доставка хотя бы один раз (он может упасть между публикацией и пометкой строки отправленной), поэтому потребителям всё равно нужна дедупликация — паттерн переносит проблему двойной записи, а не убирает потребность в идемпотентных потребителях.",
      "Наивный опрашивающий релей добавляет и задержку (события ждут до интервала опроса), и постоянную нагрузку на чтение таблицы outbox; чтобы этого избежать, обычно переходят на инфраструктуру считывания журнала."
    ],
    "tradeoffs": [
      "Опрашивающий релей (просто реализовать, добавляет задержку в размере интервала опроса и постоянную нагрузку на чтение) против релея на основе чтения журнала через change data capture (почти нулевая добавленная нагрузка и задержка, но нужна инфраструктура доступа к журналу).",
      "Удаление строк после публикации (таблица остаётся компактной) против пометки published_at и хранения строк (полный аудиторский след ценой роста таблицы, которым нужно отдельно управлять).",
      "Опора на порядок по первичному ключу/вставке для порядка публикации (просто, работает в рамках одного агрегата) против явного упорядочивания по агрегату, когда несколько писателей могут одновременно вставлять строки outbox для одного и того же агрегата."
    ],
    "whenToUse": [
      "Любой путь записи, который должен и сохранить состояние, и надёжно уведомить другие сервисы или системы, — оформление заказа, списание платежа, изменение остатков.",
      "Замена существующего прямого пути «записать в БД, затем вызвать брокер», для которого в проде уже наблюдалось расхождение после сбоя или простоя брокера.",
      "Питание оркестратора саги или событийных потребителей записями, сделанными внутри в остальном обычной реляционной транзакции."
    ],
    "whenNotToUse": [
      "Однопроцессные системы без потребителей ниже по потоку — уведомлять некого, поэтому дополнительная таблица и релей — чистые накладные расходы.",
      "Системы, у которых уже есть надёжная, хорошо эксплуатируемая распределённая транзакция (2PC), охватывающая базу и брокер, — на практике редкость, но там, где она реально есть и ей доверяют, таблица outbox добавляет второй механизм для той же задачи."
    ]
  },
  "delivery-semantics": {
    "tagline": "Доставка ровно один раз — это не то, что даёт сеть сама по себе, а доставка хотя бы один раз плюс дедупликация по id сообщения.",
    "definition": "Семантики доставки описывают, что система обмена сообщениями обещает по поводу того, сколько раз потребитель увидит каждое сообщение. At-most-once отправляет сообщение без повторов, поэтому оно может быть молча потеряно. At-least-once повторяет отправку, пока не получит подтверждение, поэтому сообщение может прийти больше одного раза. «Ровно один раз» — это то, чего на самом деле хотят люди: у каждого сообщения ровно один эффект, — но ни один реальный транспорт не даёт этого бесплатно; такая семантика строится поверх доставки хотя бы один раз плюс потребитель, который распознаёт и пропускает уже применённые сообщения по стабильному id.",
    "problem": "Брокер, настроенный на «ровно один раз», всё равно вынужден повторять отправку: потребитель может упасть после применения эффекта сообщения, но до подтверждения, или сетевая задержка может отложить подтверждение дольше таймаута, который запускает повторную доставку. В любом случае одно и то же сообщение доходит до обработчика дважды. Если обработчик не рассчитан на это, бизнес-эффект происходит дважды — клиента списывают дважды, письмо с уведомлением отправляется дважды, счётчик увеличивается дважды, — и это выглядит как баг брокера, хотя брокер повёл себя ровно так, как спроектирован.",
    "solution": "Относиться к каждому потребителю так, будто он получает сообщения хотя бы один раз, и явно распознавать повторную доставку: перед применением эффекта сообщения проверять, не записан ли уже его id как обработанный; если да — пропустить; если нет — применить эффект и записать id атомарно, в одной транзакции. Это превращает «брокер гарантирует ровно один раз» — обещание, которое ни одна реальная система полностью не держит, — в «потребитель гарантирует фактически один раз», что достижимо обычной транзакцией базы данных и сочетается с идемпотентностью.",
    "code": "// Обработка «ровно один раз», построенная из доставки хотя бы один раз плюс дедупликация по id.\nasync function handleMessage(db: Db, msg: { id: string; type: string; payload: unknown }) {\n  const already = await db.query('SELECT 1 FROM processed_messages WHERE id = $1', [msg.id]);\n  if (already.length > 0) return; // повторная доставка уже применённого сообщения\n\n  await db.transaction(async (tx) => {\n    await applySideEffect(tx, msg); // например, зачислить средства на счёт\n    await tx.insert('processed_messages', { id: msg.id, processedAt: new Date() });\n  });\n}",
    "pros": [
      "Даёт именно ту гарантию на уровне бизнеса, которую хотят получить («это произошло один раз»), без необходимости, чтобы сам брокер поддерживал настоящую доставку ровно один раз.",
      "Сочетается с любым брокером с доставкой хотя бы один раз — Kafka, SQS, RabbitMQ с ручным подтверждением, — без привязки к фирменной функции «exactly-once» конкретного вендора.",
      "Превращает повторную доставку в ожидаемый, протестированный путь кода вместо редкого краевого случая, обнаруженного в проде как баг с двойным списанием."
    ],
    "cons": [
      "Требует долговечной транзакционной записи обработанных id рядом с побочным эффектом — если эти два действия записываются раздельно (не атомарно), сбой между ними заново открывает ту самую гонку, которую механизм должен был закрыть.",
      "Продюсеры обязаны присылать стабильные уникальные id, переживающие собственные повторы, иначе потребителю не на чем строить дедупликацию.",
      "Таблица обработанных id растёт без ограничений, если её не чистить по окну хранения, соответствующему максимально реалистичной задержке повторной доставки."
    ],
    "tradeoffs": [
      "At-most-once (никогда не повторяет отправку, поэтому может молча терять сообщения) против at-least-once с дедупликацией (всегда повторяет, нужна таблица обработанных id) — большинство систем принимают цену дедупликации, потому что молчаливая потеря обычно хуже.",
      "Дедупликация по стабильному id сообщения (дёшево, требует от продюсера гарантии стабильности id) против дедупликации по хешу содержимого или бизнес-ключу идемпотентности (работает даже со слабым продюсером, но дороже вычислять и сравнивать).",
      "Подтверждение после применения эффекта (безопасно — сбой в середине обработки вызывает повторную доставку, с которой справляется дедупликация) против подтверждения до применения эффекта (никогда не доставляется повторно, но сбой сразу после подтверждения теряет сообщение безвозвратно)."
    ],
    "whenToUse": [
      "Любой потребитель брокера или очереди с доставкой хотя бы один раз — Kafka, SQS, RabbitMQ с ручным подтверждением, — где повторная доставка задокументирована как ожидаемая возможность.",
      "Обработчики, чей побочный эффект не идемпотентен сам по себе, — списание денег, отправка письма, увеличение счётчика.",
      "Объяснение стейкхолдерам, почему изредка повторяющееся письмо — ожидаемое поведение недедуплицирующего потребителя, а не сбой брокера."
    ],
    "whenNotToUse": [
      "Обработчики, уже идемпотентные сами по себе, — upsert по собственному id сообщения не нуждается в отдельной таблице дедупликации.",
      "Малоценные, терпимые к потере сигналы — метрики или телеметрия по принципу best-effort, — где at-most-once и редкая молчаливая потеря являются более дешёвым приемлемым выбором."
    ]
  },
  "optimistic-locking": {
    "tagline": "Считать конфликты редкими: писать только если строка не менялась с момента чтения, и отдавать вызывающей стороне явный конфликт вместо молчаливой перезаписи.",
    "definition": "Оптимистичная блокировка (optimistic concurrency control) прикрепляет к каждой строке номер версии — или временную метку, используемую тем же способом. Писатель читает строку вместе с её текущей версией, вычисляет новое значение и записывает его условно: UPDATE ... WHERE id = ? AND version = ?. Если за это время строку никто больше не трогал, версия по-прежнему совпадает, обновление проходит и версия увеличивается. Если кто-то уже обновил строку раньше, условию WHERE не соответствует ни одна строка, и писатель получает явный сигнал — а не молчаливую перезапись, — что нужно перечитать данные, свести изменения или спросить пользователя, что делать.",
    "problem": "Два клиента читают одну и ту же строку почти одновременно, каждый вычисляет изменение на основе этого устаревшего снимка, и оба записывают результат обратно. Без обнаружения конфликта вторая запись просто перезаписывает первую — побеждает тот, кто записал последним, а изменение первого писателя теряется без следа, без ошибки, незамеченным до тех пор, пока кто-то не обнаружит это позже. Это классическая проблема потерянного обновления (lost update), и она особенно вероятна там, где пользователь читает запись, тратит время на размышление или редактирование и только потом отправляет запись, — именно это «время на размышление» и есть окно, в которое может вклиниться другой писатель.",
    "solution": "Добавить к строке колонку версии (или переиспользовать updated_at как версию де-факто). Каждая запись включает прочитанную версию как условие обновления и увеличивает версию в рамках того же запроса. Если условное обновление не затронуло ни одной строки, значит кто-то другой уже продвинул версию вперёд с момента чтения — запись не произошла, и вызывающая сторона получает это как явный результат, который нужно обработать: перечитать данные и повторить, показать интерфейс слияния или вывести ошибку, вместо того чтобы строку молча затёрли и никто об этом не узнал.",
    "code": "// Оптимистичная конкурентность: UPDATE проходит только если версия не сдвинулась.\nasync function updateProfile(db: Db, id: string, expectedVersion: number, patch: Partial<Profile>) {\n  const result = await db.query(\n    'UPDATE profiles SET name = $1, version = version + 1 WHERE id = $2 AND version = $3',\n    [patch.name, id, expectedVersion],\n  );\n\n  if (result.rowCount === 0) {\n    throw new ConflictError('профиль был изменён кем-то другим с момента вашего чтения');\n  }\n}",
    "pros": [
      "Пока пользователь читает, думает или редактирует, блокировка не удерживается — другие читатели и писатели не блокируются на всё время редактирования, только на мгновение финальной записи.",
      "Конфликты проявляются как явный результат, который вызывающая сторона обязана обработать, заменяя молчаливое потерянное обновление видимым, тестируемым сценарием отказа.",
      "Дёшево в обычном случае без конкуренции: один условный UPDATE, без отдельного round-trip на получение и освобождение блокировки."
    ],
    "cons": [
      "При высокой конкуренции за одну и ту же строку большинство записей проигрывают гонку и вынуждены повторяться, что может работать хуже пессимистичной блокировки, которая просто ставит писателей в очередь вместо того, чтобы отклонять и повторять их.",
      "В обновление строки должен участвовать каждый путь записи — включать версию в WHERE и увеличивать её, — иначе один писатель, пропустивший это, молча возвращает проблему потерянного обновления, которую механизм должен был закрыть.",
      "Сообщает только то, что строка изменилась, но не что именно изменилось — разрешение конфликта (слияние, отклонение, повтор) полностью остаётся на вызывающей стороне."
    ],
    "tradeoffs": [
      "Оптимистичная блокировка (без блокирования, повтор при конфликте, лучше всего при редких конфликтах между писателями) против пессимистичной блокировки (блокирует других писателей заранее, лучше при частых или ожидаемых конфликтах).",
      "Единый счётчик версии (просто, сообщает только что строка изменилась) против сравнения по колонкам или CRDT-подобной структуры (сообщает, что именно изменилось, и иногда позволяет слить автоматически вместо отказа).",
      "Явный отказ запроса при конфликте (просто, перекладывает повтор или диалог с пользователем на вызывающую сторону) против автоматического повтора со свежим чтением (скрывает конфликт от пользователя, но рискует штормом повторов при реально высокой конкуренции)."
    ],
    "whenToUse": [
      "Многошаговое редактирование, где пользователь читает, думает, затем пишет, — веб-формы, редакторы документов, — где удержание блокировки на всё время размышления заблокировало бы всех остальных.",
      "Сущности, где конфликтующие конкурентные записи на практике редки, поэтому платить за блокировку заранее не оправдано ради редкого конфликта.",
      "Замена существующего молчаливого пути «побеждает последняя запись», где проблема потерянного обновления уже наблюдалась в проде."
    ],
    "whenNotToUse": [
      "Горячие строки с высокой конкуренцией — общий счётчик, остаток товара у одного популярного лота, — где большинство конкурентных записей будут конфликтовать; атомарный инкремент, очередь или пессимистичная блокировка избегают возникающего шторма повторов.",
      "Пути, где у вызывающей стороны реально нет разумного способа отреагировать на конфликт, — фоновая задача fire-and-forget, которой некому показать сообщение «кто-то другой это изменил»."
    ]
  },
  "backpressure": {
    "tagline": "Когда продюсер обгоняет потребителя, неограниченная очередь не предотвращает падение — она лишь откладывает его и делает хуже.",
    "definition": "Backpressure — это явная политика того, что система делает, когда входящая работа поступает быстрее, чем успевает обрабатываться: ограничить очередь или буфер между продюсером и потребителем и, когда он заполнен, либо отклонять новую работу (сигнализируя вызывающей стороне снизить темп), либо вытеснять существующую работу с более низким приоритетом, чтобы освободить место. Любой из вариантов удерживает память и задержку в предсказуемых границах независимо от размера входящего всплеска, вместо того чтобы позволить буферу расти бесконечно.",
    "problem": "Быстрый продюсер — всплеск трафика, шторм повторов, запустившаяся batch-задача — кладёт работу в очередь быстрее, чем более медленный потребитель успевает её вычерпывать. Если у очереди нет ограничения, она продолжает расти: потребление памяти увеличивается, а каждый следующий элемент ждёт дольше предыдущего, так что к моменту, когда элемент наконец обрабатывается, он может быть уже настолько устаревшим, что не имеет смысла. В итоге процессу не хватает памяти, и он падает, утягивая с собой всю работу, включая запросы, которые иначе успешно бы завершились. То, что выглядело как изящная обработка всплеска, превращается в медленный, самопричинённый и куда более масштабный простой, чем простое отклонение избытка.",
    "solution": "Дать очереди фиксированную вместимость и явную политику того, что происходит при заполнении: немедленно отклонять новый элемент, возвращая ошибку, которую вызывающая сторона видит и на которую может отреагировать (повторить позже, снизить темп, быстро завершиться с отказом), либо вытеснить существующий элемент — обычно самый старый или наименее приоритетный, — чтобы принять что-то более ценное. В любом случае система отказывает быстро, дёшево и заметно при ограниченном размере, вместо того чтобы отказывать медленно, дорого и незаметно, когда закончится память.",
    "code": "// Ограниченная очередь: отклонять новую работу по достижении вместимости, а не расти бесконечно.\nclass BoundedQueue<T> {\n  private items: T[] = [];\n  constructor(private readonly capacity: number) {}\n\n  tryPush(item: T): boolean {\n    if (this.items.length >= this.capacity) {\n      return false; // сигнализируем backpressure вызывающей стороне — не растём бесконечно\n    }\n    this.items.push(item);\n    return true;\n  }\n\n  pop(): T | undefined {\n    return this.items.shift();\n  }\n}\n\nasync function handleRequest(queue: BoundedQueue<Job>, job: Job) {\n  if (!queue.tryPush(job)) {\n    throw new Error('503: перегрузка, повторите позже'); // быстрый отказ, вызывающий может снизить темп\n  }\n}",
    "pros": [
      "Отказ становится немедленным и дешёвым — отклонить один запрос — вместо отложенного и катастрофического: падения из-за нехватки памяти, утягивающего весь процесс и все уже выполняющиеся запросы.",
      "Делает перегрузку видимой для вызывающей стороны как явный отказ, на который можно отреагировать — повторить в другом месте, снизить темп, поднять алерт, — вместо того чтобы прятать её внутри постоянно растущей, незаметно деградирующей задержки.",
      "Удерживает потребление памяти и задержку обработки в предсказуемых границах при любой входящей нагрузке, а не только при той, на которой систему случайно тестировали в staging."
    ],
    "cons": [
      "Отклонённой или вытесненной работе нужно куда-то деваться — она отбрасывается, повторяется вызывающей стороной или перенаправляется, — иначе backpressure просто переносит проблему перегрузки на то, что находится выше по потоку.",
      "Выбор вместимости — реальная задача настройки: слишком маленькая отклоняет всплески, которые вполне можно было бы поглотить; слишком большая лишь откладывает тот же самый отказ из-за нехватки памяти, который механизм должен предотвращать.",
      "Вытеснение по приоритету (а не простой FIFO) требует, чтобы система вообще знала приоритет каждого элемента, — это логика классификации, которая простой ограниченной очереди раньше не требовалась."
    ],
    "tradeoffs": [
      "Прямое отклонение новой работы (просто, защищает всё уже принятое, наказывает того, кому не повезло прийти во время всплеска) против вытеснения старых или низкоприоритетных элементов (защищает свежесть, но может отбросить работу, уже частично обработанную).",
      "Маленький буфер (быстрый отказ, низкая задержка для принятого, отклоняет большую часть всплеска) против большого буфера (поглощает более крупные всплески, но элементы в очереди ждут дольше и могут устареть к моменту обработки).",
      "Сигнализация backpressure только непосредственному вызывающему (просто, локально) против распространения её по всей цепочке вызовов, вместе с изоляцией bulkhead и отключением circuit breaker, так что системы выше по потоку тоже снижают темп, вместо того чтобы давление накапливалось только в одной точке."
    ],
    "whenToUse": [
      "Любая очередь, буфер или канал между продюсером и более медленным потребителем, где возможны всплески, — пайплайны приёма данных, внутрипроцессные очереди задач, HTTP-серверы под нагрузкой.",
      "Системы, которые уже падали или катастрофически деградировали при всплеске трафика, вместо того чтобы аккуратно отказывать ограниченной доле запросов.",
      "Защита медленной нижестоящей зависимости от перегрузки быстрой вышестоящей, обычно вместе с изоляцией bulkhead и отключением circuit breaker на каждом хопе."
    ],
    "whenNotToUse": [
      "Фиксированная, хорошо понятная нагрузка с большим запасом — внутренняя batch-задача с известным ограниченным объёмом входных данных, — где операционная цена настройки вместимости почти ничего не даёт.",
      "Пути, где потеря или отклонение работы неприемлемы, а медленная обработка вполне устраивает, — очередь, питающая ночной отчёт, — где неограниченная (или очень большая) очередь и есть осознанное намерение, а не ошибка."
    ]
  },
  "change-data-capture": {
    "tagline": "Читать собственный журнал упреждающей записи базы данных вместо опроса таблиц — и видеть каждое закоммиченное изменение ровно один раз, в порядке коммитов.",
    "definition": "Change data capture (CDC) — техника превращения закоммиченных записей базы данных в поток упорядоченных событий изменений путём считывания того же журнала транзакций (write-ahead log, binlog), который база данных и так ведёт для собственной репликации, — вместо периодических запросов к таблицам в поисках того, что изменилось. Каждое событие изменения несёт номер последовательности в журнале (LSN), монотонно возрастающий вместе с порядком коммитов, поэтому потребители ниже по потоку могут обрабатывать события строго в том порядке, в котором они реально закоммитились, и после рестарта возобновлять работу точно с того LSN, на котором остановились.",
    "problem": "Опрос таблицы на предмет «что изменилось с момента последней проверки» одновременно теряет данные и подвержен гонкам: если строка обновляется дважды между двумя опросами, опрос видит только финальное состояние, а все промежуточные значения исчезают без следа. Строки, обновлённые и затем удалённые между опросами, могут пропасть целиком, без единой записи о том, что они вообще менялись. А опрос, идущий вперегонки с конкурентными записями, может пропустить строку, закоммиченную сразу после начала запроса, или посчитать дважды ту, что закоммитилась в середине сканирования. Вдобавок к проблемам корректности, опрос добавляет нагрузку на чтение основной базе данных на каждом интервале — независимо от того, изменилось ли вообще что-нибудь.",
    "solution": "CDC-коннектор читает журнал транзакций базы данных напрямую — ту же самую долговечную упорядоченную запись, на которую опирается сама база для репликации, — вместо того чтобы выполнять запросы к таблицам. Каждое закоммиченное изменение становится событием ровно один раз, в истинном порядке коммитов, помеченным своим LSN. Потребители ниже по потоку применяют события в порядке LSN и сохраняют последний успешно применённый LSN вместе со своим состоянием, поэтому после падения или рестарта они возобновляют работу точно с того места, где остановились, вместо того чтобы гадать, что могло быть пропущено.",
    "code": "// CDC-потребитель: упорядочивает события по LSN и возобновляет работу точно с того места, где остановился.\ninterface ChangeEvent { table: string; lsn: bigint; op: 'insert' | 'update' | 'delete'; row: unknown; }\n\nclass CdcConsumer {\n  private lastLsn = 0n;\n\n  async apply(event: ChangeEvent) {\n    if (event.lsn <= this.lastLsn) return; // уже обработано — безопасно пропустить при повторной доставке\n    await this.project(event);\n    this.lastLsn = event.lsn; // сохраняется вместе с проекцией, а не только в памяти\n  }\n\n  private async project(event: ChangeEvent) {\n    // обновить read-модель / поисковый индекс / кэш на основе изменения строки\n  }\n}",
    "pros": [
      "Захватывает каждое закоммиченное изменение ровно один раз в истинном порядке коммитов, включая промежуточные состояния, которые опрашивающий запрос молча пропустил бы.",
      "Добавляет почти нулевую дополнительную нагрузку на исходную базу данных — коннектор читает журнал, который база и так пишет для собственной репликации, а не дополнительные прикладные запросы.",
      "Полностью отделяет нижестоящие системы — поисковый индекс, кэш, хранилище данных, read-модель другого сервиса — от схемы и шаблонов запросов исходной базы."
    ],
    "cons": [
      "Требует доступа к журналу и CDC-коннектора (Debezium или встроенной логической репликации базы данных) как реального эксплуатируемого компонента инфраструктуры — он должен работать, находиться под наблюдением и обновляться вместе с исходной базой.",
      "Изменения схемы исходной таблицы — переименованная колонка, изменённый тип — могут сломать поток изменений или любого потребителя ниже по потоку, рассчитывавшего на старую форму данных.",
      "Потребители всё равно видят изменения с некоторой задержкой репликации и должны решать те же вопросы «возобновления с последнего LSN» и дедупликации, что и при работе с любым другим потоком событий, — CDC устраняет проблемы опроса, а не все проблемы распределённых систем вообще."
    ],
    "tradeoffs": [
      "CDC на основе журнала (почти нулевая нагрузка на источник, истинный порядок коммитов, требует инфраструктуры доступа к журналу/коннектора) против опроса изменений (работает с любой базой данных без специального доступа, но теряет данные и добавляет постоянную нагрузку на чтение).",
      "Потоковая передача каждого изменения на уровне строки (полная история, больше событий, больше обработки ниже по потоку) против периодического снапшота с сравнением (меньше событий, дешевле, но более грубая гранулярность и слепота к промежуточным состояниям между снапшотами).",
      "Доставка потребителям хотя бы один раз, требующая дедупликации по LSN, — простой контракт коннектора, больше учётной работы на стороне потребителя, — против sink-коннекторов с доставкой ровно один раз, которые упрощают потребителя ценой более сложной и хрупкой машинерии на стороне коннектора."
    ],
    "whenToUse": [
      "Поддержание поискового индекса, кэша или оптимизированной для чтения проекции в синхронизации с таблицей системы записи без двойной записи из прикладного кода.",
      "Питание событийного пайплайна или хранилища данных другого сервиса из существующей реляционной базы данных, изначально не спроектированной событийно.",
      "Аудит или воспроизведение полной истории изменений таблицы, а не только осмотр её текущего состояния."
    ],
    "whenNotToUse": [
      "Простые случаи, уже хорошо обслуживаемые периодическим пакетным экспортом или ETL-задачей по расписанию, удовлетворяющей требованиям к свежести данных, — инфраструктура считывания журнала для CDC там неоправданна.",
      "Исходные системы, вообще не предоставляющие стабильный журнал или поток логической репликации (некоторые управляемые базы данных это ограничивают), — опрос или прикладной transactional outbox могут оказаться единственным доступным вариантом."
    ]
  }
};
...
export const questionProse: Record<string, QuestionProse> = {
  ...
  "data-cap-theorem-1": {
    "prompt": "Согласно теореме CAP, между чем вынуждено выбирать распределённое хранилище данных при возникновении сетевой партиции?",
    "options": [
      "Между согласованностью (каждое чтение видит последнюю запись) и доступностью (каждый запрос получает ответ без ошибки), поскольку от устойчивости к партициям в реальной сети отказаться нельзя",
      "Между согласованностью и устойчивостью к партициям, потому что доступность всегда обеспечивается повторными попытками клиента",
      "Между задержкой и пропускной способностью, поскольку CAP по сути о производительности под нагрузкой",
      "Между безопасностью и удобством использования, поскольку шифрование реплицируемых данных замедляет консенсус"
    ],
    "explanation": "CAP действует именно во время партиции: хранилище не может одновременно давать гарантированно свежий ответ и гарантированно безошибочный ответ на каждый запрос по обе стороны разрыва, а устойчивость к партициям не опциональна в реальной сети с реальными отказами. Второй вариант путает доступность со стратегией повторов — повторные попытки не восстанавливают ответ, которого, по теореме, не может существовать во время партиции. Третий принимает CAP за модель производительности; этот компромисс ближе к тому, что PACELC добавляет поверх CAP. Четвёртый выдумывает компромисс, о котором CAP ничего не говорит."
  },
  "data-cap-theorem-2": {
    "prompt": "Команда выбирает строгую согласованность (CP) для сервиса корзины покупок, чтобы он оставался корректным во время сетевой партиции. Какова прямая цена этого выбора?",
    "options": [
      "Во время партиции сторона-меньшинство должна отказывать в запросах или блокировать их, пока не станет достижим кворум, поэтому часть пользователей видит ошибки или таймауты",
      "Сервис становится eventually consistent и молча возвращает устаревшие корзины",
      "Сервис полностью теряет устойчивость к партициям и падает при любом сбое сети",
      "Цены нет — CP даёт одновременно мгновенные ответы и всегда свежие данные"
    ],
    "explanation": "Выбор CP означает, что сторона партиции, не способная достичь кворума, обязана перестать отвечать, а не отвечать потенциально неверными данными — именно эта недоступность, а не тихая устарелость, и есть цена. Второй вариант описывает выбор AP — противоположность сделанному выбору. Третий преувеличивает: CP-системы продолжают работать на стороне большинства, отказывая в обслуживании лишь меньшинству. Четвёртый игнорирует саму суть CAP — то, что во время партиции бесплатного выбора не существует."
  },
  "data-cap-theorem-3": {
    "prompt": "Этот клиент переключается на реплику и помечает чтение как `stale`, когда основной узел не отвечает вовремя. Какой выбор CAP это представляет и почему?",
    "code": "class PartitionAwareClient {\n  async read(): Promise<Reading> {\n    try {\n      return await this.withTimeout(this.primary.read(), this.timeoutMs);\n    } catch {\n      const r = await this.replica.read();\n      return { ...r, stale: true };\n    }\n  }\n}",
    "options": [
      "AP — жертвует строгой согласованностью, чтобы продолжать отвечать на запросы во время того, что выглядит как партиция",
      "CP — блокирует вызывающего до тех пор, пока основной узел снова не станет доступен",
      "Это вообще обходит CAP стороной, поскольку клиентский таймаут — не то же самое, что сетевая партиция",
      "CA — гарантирует одновременно полную согласованность и полную доступность"
    ],
    "explanation": "Отдавать последнее известное значение реплики вместо блокировки — это выбор доступности: клиент продолжает отвечать ценой гарантированно свежего ответа и явно помечает эту цену полем `stale`. Второй вариант описывает противоположное поведение — блокировка это и есть выбор CP, а не то, что делает этот код. Третий неверен: с точки зрения клиента недоступный в пределах таймаута основной узел неотличим от партиции и трактуется именно так. Четвёртый описывает сочетание, которое CAP доказывает невозможным удержать на протяжении реальной партиции."
  },
  "data-consistency-models-1": {
    "prompt": "Что гарантирует согласованность read-your-writes и чего она не гарантирует?",
    "options": [
      "Что клиент увидит свои собственные предыдущие записи при последующих чтениях, но ничего не гарантирует о том, что и когда увидят другие клиенты",
      "Что все клиенты повсюду увидят каждую запись в одном и том же глобальном порядке (линеаризуемость)",
      "Что записи в конце концов дойдут до всех реплик, без гарантии, какой клиент что увидит первым",
      "Что чтения всегда обслуживаются с primary и никогда с реплики"
    ],
    "explanation": "Read-your-writes — это гарантия уровня сессии: она обещает лишь то, что клиент не увидит исчезновения своих собственных записей, и ничего не говорит о порядке между разными клиентами. Второй вариант описывает линеаризуемость — намного более сильную и дорогую гарантию. Третий описывает обычную eventual consistency, которая слабее read-your-writes и не защищает клиента от исчезновения его собственной записи. Четвёртый описывает политику маршрутизации, а не модель согласованности — read-your-writes можно обеспечить, направляя чтение на любую достаточно догнавшую реплику, а не только на primary."
  },
  "data-consistency-models-2": {
    "prompt": "Почему команда может сознательно выбрать eventual consistency вместо линеаризуемости для счётчика «лайков», но не для баланса счёта?",
    "options": [
      "Eventual consistency меняет небольшое, в основном безобидное окно устарелости на меньшую задержку и большую доступность — приемлемо для счётчика, неприемлемо для денег",
      "Eventual consistency строго более корректна в любом случае, поэтому её стоит предпочитать всегда, когда она доступна",
      "Линеаризуемость реализуема только для числовых полей, поэтому у баланса нет другого выбора",
      "Реальной разницы между моделями нет — они возвращают одинаковый результат при любой нагрузке"
    ],
    "explanation": "Выбор — это расчёт затрат и выгод: немного устаревший счётчик лайков незаметен пользователю, а немного устаревший баланс может означать трату денег, которых на самом деле нет, поэтому более сильная и дорогая гарантия оправдана именно для баланса. Второй вариант перевёрнут — eventual consistency слабее, а не более корректна. Третий выдумывает несуществующее ограничение: линеаризуемость применима к любому типу данных. Четвёртый отрицает саму предпосылку существования моделей согласованности."
  },
  "data-consistency-models-3": {
    "prompt": "Код направляет чтение клиента только на реплику, версия которой не меньше версии последней записи клиента. Какую именно аномалию это предотвращает?",
    "code": "class SessionConsistentReader {\n  private lastWrittenVersion = 0;\n  recordWrite(version: number): void {\n    this.lastWrittenVersion = Math.max(this.lastWrittenVersion, version);\n  }\n  read(key: string, replicas: Replica[]): string | undefined {\n    const caughtUp = replicas.find((r) => r.version() >= this.lastWrittenVersion);\n    if (!caughtUp) throw new Error('no replica is caught up yet');\n    return caughtUp.read(key);\n  }\n}",
    "options": [
      "Собственная недавняя запись клиента как будто исчезает, потому что чтение попало на реплику, ещё не догнавшую эту запись",
      "Два разных клиента навсегда расходятся в представлении об итоговом значении",
      "Запись теряется полностью, ещё не дойдя ни до одной реплики",
      "Взаимная блокировка (deadlock) между двумя параллельными транзакциями"
    ],
    "explanation": "Отказываясь читать с реплики, не догнавшей версию последней записи клиента, код гарантирует, что клиент всегда увидит свои собственные записи — это и есть гарантия read-your-writes. Он не решает постоянное расхождение между разными клиентами (для этого нужен механизм разрешения конфликтов или сходимости), не защищает от того, что запись вообще не дойдёт ни до одной реплики (это вопрос долговечности/фактора репликации), и не имеет отношения к deadlock, который возникает из-за порядка блокировок, а не из-за маршрутизации чтения."
  },
  "data-quorum-1": {
    "prompt": "Почему для кворумных чтений и записей R + W должно быть больше N, чтобы гарантировать, что чтение увидит последнюю подтверждённую запись?",
    "options": [
      "Это заставляет любой набор чтения пересекаться с любым набором записи хотя бы в одном узле, поэтому хотя бы одна из затронутых чтением реплик имеет последнюю запись",
      "Это гарантирует, что все N реплик всегда полностью синхронизированы в каждый момент времени",
      "Это произвольное соглашение, не влияющее на корректность, а только на производительность",
      "Это гарантирует, что записи применяются в одном и том же порядке на каждом узле"
    ],
    "explanation": "R + W > N — это аргумент по принципу Дирихле: при N узлах всего набор чтения размера R и набор записи размера W, которые в сумме превышают N, не могут одновременно избежать общего узла, поэтому чтение обязательно затронет хотя бы один узел с последней записью. Это не требует полной синхронизации всех N реплик — это более сильное и ненужное свойство. Это не произвольно: нарушение условия (R + W <= N) ломает гарантию «чтение видит последнюю запись». И это ничего не говорит о порядке применения записей на узлах — это отдельный вопрос, не связанный с пересечением наборов."
  },
  "data-quorum-2": {
    "prompt": "Команда выставляет W=1 и R=1 на кворумном хранилище из 3 узлов ради максимальной скорости. Чем они жертвуют?",
    "options": [
      "Гарантией R + W > N (1 + 1 не больше 3), поэтому чтение может попасть на узел, ещё не увидевший последнюю запись",
      "Ничем — малые значения R и W влияют только на пропускную способность, но никогда на корректность",
      "Возможностью пережить хоть какой-то отказ узла",
      "Read repair, который работает только при R + W > N"
    ],
    "explanation": "При W=1 и R=1 на N=3 получаем R + W = 2 <= 3, поэтому набор чтения и набор записи могут вообще не пересечься, и клиент может прочитать устаревшие данные без какой-либо иной гарантии. Второй вариант — именно то, чем жертвует такая настройка. Третий преувеличивает: система всё ещё может переживать отказы узлов, просто больше не гарантирует свежесть чтения после записи. Четвёртый тоже неверен — read repair всё ещё может сработать, если чтение случайно затронет устаревшую реплику, просто не гарантировано срабатывать до того, как чтение вернёт последнее значение."
  },
  "data-quorum-3": {
    "prompt": "Собрав R ответов, этот код дозаписывает последнюю версию на любой узел, ответивший более старой. Как называется эта техника и что она даёт?",
    "code": "read(key: string): string | undefined {\n  const responses = this.nodes.slice(0, this.r).map((n) => ({ node: n, res: n.read(key) }));\n  const latest = responses.reduce((best, cur) =>\n    (cur.res?.version ?? -1) > (best.res?.version ?? -1) ? cur : best);\n  responses\n    .filter((r) => (r.res?.version ?? -1) < latest.res!.version)\n    .forEach((r) => r.node.write(key, latest.res!.value, latest.res!.version));\n  return latest.res?.value;\n}",
    "options": [
      "Read repair — позволяет устаревшим репликам догонять попутно, поэтому будущим чтениям требуется меньше «ремонта»",
      "Двухфазный коммит — гарантирует атомарность записи на всех узлах",
      "Выбор лидера — решает, какой узел станет primary",
      "Шардирование — делит пространство ключей между узлами"
    ],
    "explanation": "Отправка самого свежего значения на узлы, ответившие устаревшим, прямо во время обычного чтения — это read repair: он лечит реплики постепенно, без отдельного процесса восстановления. Это не имеет отношения к двухфазному коммиту, который координирует единую атомарную запись между участниками до её применения где бы то ни было. Это не выбор лидера — здесь никакой primary не выбирается. И это не шардирование, которое распределяет ключи между узлами, а не чинит устаревшие копии одного и того же ключа."
  },
  "data-partitioning-1": {
    "prompt": "В чём главная операционная проблема шардирования данных по схеме `id % N`?",
    "options": [
      "Добавление или удаление узла меняет N, из-за чего переназначается почти каждый ключ на другой шард, вызывая практически полное перемешивание данных",
      "Она ни при каких обстоятельствах не может равномерно распределить нагрузку",
      "Она требует центрального координатора для обслуживания каждого чтения",
      "Она работает только с числовыми id и никогда не может быть адаптирована под строковые ключи"
    ],
    "explanation": "Поскольку назначение шарда — прямая функция от N, изменение числа узлов сразу меняет назначение почти для всех ключей — это и есть то дорогое перемешивание, которого призван избежать consistent hashing. Шардирование по модулю на самом деле равномерно распределяет хорошо захешированные id, поэтому утверждение о неравномерности ложно. Оно не требует центрального координатора — маршрутизация это чистая функция, которую каждый узел вычисляет локально. А строковые ключи хешируются в число ничуть не хуже, чем используется числовой id напрямую."
  },
  "data-partitioning-2": {
    "prompt": "Consistent hashing обычно предпочитают `id % N` для кластеров, которые со временем будут расти. Чем приходится пожертвовать ради этой выгоды?",
    "options": [
      "Простотой — нужно кольцо хешей с виртуальными узлами вместо одной операции взятия остатка, взамен на локальность перемещения данных при решардинге",
      "Ничем — consistent hashing строго проще операции взятия остатка во всех отношениях",
      "Возможностью вообще добавлять или удалять узлы кластера",
      "Равномерным распределением ключей, которое может гарантировать только схема с остатком"
    ],
    "explanation": "Consistent hashing меняет немного реализационной и концептуальной сложности — кольцо, виртуальные узлы, поиск по кольцу — на намного меньшую стоимость решардинга при изменении размера кластера. Это не проще во всех отношениях: кольцо — это действительно больше механики, чем одна операция взятия остатка. Это не лишает возможности менять размер кластера — как раз наоборот, в этом весь смысл. А равномерное распределение достижимо и с consistent hashing — за счёт достаточного числа виртуальных узлов на физический узел; у схемы с остатком нет на это монополии."
  },
  "data-partitioning-3": {
    "prompt": "Почему добавление узла в это кольцо перемещает лишь малую долю ключей, в отличие от `id % N`?",
    "code": "class ConsistentHashRing {\n  private ring: { hash: number; node: string }[] = [];\n  addNode(node: string, points = 3): void {\n    for (let i = 0; i < points; i++) {\n      this.ring.push({ hash: this.hash(`${node}#${i}`), node });\n    }\n    this.ring.sort((a, b) => a.hash - b.hash);\n  }\n  shardFor(key: string): string {\n    const h = this.hash(key);\n    return (this.ring.find((p) => p.hash >= h) ?? this.ring[0]).node;\n  }\n}",
    "options": [
      "Ключ перемещается, только если новый узел встаёт между ним и его текущим владельцем на кольце — ближайший по часовой стрелке узел для всех остальных ключей не меняется",
      "Потому что кольцо пересчитывает хеш каждого ключа заново при каждом добавлении узла",
      "Потому что виртуальные точки делают кольцо полностью невосприимчивым к любому перемещению данных",
      "Потому что кольцу разрешено содержать только ровно один узел"
    ],
    "explanation": "Consistent hashing переназначает только те ключи, что попадают между позицией нового узла на кольце и позицией его предшественника — для всех остальных ближайший по часовой стрелке узел не меняется, именно поэтому миграция локальна, а не глобальна. Хеши ключей не пересчитываются при изменении узлов — меняется только назначение узел-ключ для затронутого среза. Виртуальные точки сглаживают распределение нагрузки, но не устраняют перемещение данных полностью — часть перемещения неизбежна и желательна, когда новый узел забирает свою долю. А кольцо поддерживает любое число узлов — в этом весь смысл его использования для растущего кластера."
  },
  "data-replication-1": {
    "prompt": "Почему чтение, направленное на реплику, может вернуть другой, более старый ответ, чем то же чтение, направленное на leader?",
    "options": [
      "Репликация на followers обычно асинхронна, поэтому follower может отставать от leader на какое-то время",
      "Реплики хранят совершенно другую схему данных, чем leader",
      "Реплики только принимают записи и вообще никогда не обслуживают чтения",
      "Leader и реплики спроектированы так, чтобы всегда расходиться, независимо от режима репликации"
    ],
    "explanation": "Асинхронная репликация подтверждает запись на leader до того, как её применит каждый follower, поэтому follower законно может отставать на миллисекунды или секунды — именно из-за этого лага его чтения могут быть устаревшими. Реплики зеркалируют схему leader, а не расходятся с ней структурно. Обслуживание чтений — как раз одна из главных причин иметь реплики вообще. А согласие — это цель репликации, а не расхождение: лаг — побочный эффект асинхронного режима, а не замысел."
  },
  "data-replication-2": {
    "prompt": "Команда переходит с асинхронной репликации на синхронную специально, чтобы устранить устаревшие чтения с реплик. Чем она за это платит?",
    "options": [
      "Более высокой задержкой записи, и запись может зависать или отказывать, если синхронный follower медленный или недоступен",
      "Ничем ощутимым — у синхронной репликации нет недостатков по сравнению с асинхронной",
      "Полной потерей возможности переключиться (failover) на follower",
      "Обязательным сокращением числа реплик ровно до одной"
    ],
    "explanation": "Синхронная репликация подтверждает запись только после того, как нужные followers её подтвердили, поэтому задержка записи на leader теперь ограничена самым медленным из требуемых followers, а застрявший или недоступный follower может полностью заблокировать запись. Реальная и хорошо известная цена существует, поэтому утверждение об отсутствии недостатков ложно. Failover при синхронной репликации становится, наоборот, безопаснее, а не невозможным, поскольку followers гарантированно догнали leader. И ничто в синхронной репликации не ограничивает число реплик одной — её можно применять к любому числу followers, требуемых для подтверждения."
  },
  "data-replication-3": {
    "prompt": "Почему этот роутер переключается на leader вместо того, чтобы просто выбрать наименее отстающую реплику, когда все реплики превышают `maxLagMs`?",
    "code": "class LagAwareReadRouter {\n  read(key: string): string | undefined {\n    const fresh = this.replicas\n      .filter((r) => r.lagMs() <= this.maxLagMs)\n      .sort((a, b) => a.lagMs() - b.lagMs());\n    if (fresh.length === 0) {\n      return this.leader.read(key);\n    }\n    return fresh[0].read(key);\n  }\n}",
    "options": [
      "Потому что «наименее плохая из плохих» реплика всё ещё может быть устаревшей больше, чем приемлемо вызывающему — только leader гарантированно актуален",
      "Потому что чтение с любой реплики всегда медленнее, чем чтение с leader",
      "Потому что leader — единственный узел, которому вообще разрешено обслуживать чтения в реплицированной системе",
      "Потому что реплики не могут обслуживать чтения, пока запись в процессе выполнения"
    ],
    "explanation": "Выбор «лучшей из плохих» всё ещё не удовлетворяет бюджету свежести вызывающего, если все реплики его превышают — только leader гарантированно отражает последнюю подтверждённую запись, поэтому роутер переключается именно на него. Чтения с реплик обычно быстрее, а не медленнее — это и есть причина предпочитать их, когда они достаточно свежи. Реплики регулярно обслуживают чтения именно в этом паттерне — это их основное назначение. И здесь ничто не связано с записью «в процессе» — проверка касается исключительно измеренного лага относительно бюджета."
  },
  "data-consensus-1": {
    "prompt": "Какую проблему решает fencing token, которую не решает обычная распределённая блокировка?",
    "options": [
      "Он не даёт лидеру, который завис (пауза GC, задержка сети) и потерял блокировку, испортить общее состояние, когда тот очнётся, — потому что хранилище отклоняет его уже устаревший токен",
      "Он устраняет необходимость в том, чтобы большинство узлов вообще о чём-либо договаривалось",
      "Он делает получение блокировки мгновенным, без единого сетевого обмена",
      "Он гарантирует, что блокировку никогда не сможет забрать конкурирующий процесс"
    ],
    "explanation": "Обычная блокировка лишь сообщает процессу, что он когда-то получил эксклюзивность; она не мешает процессу, зависшему дольше срока аренды, позже действовать так, будто он всё ещё её держит. Монотонно растущий fencing token позволяет слою хранения отклонять любую запись со старым токеном, закрывая именно этот пробел. Он не устраняет необходимость в согласии большинства — сам токен выдаётся сервисом на основе консенсуса, которому всё ещё нужно большинство. Он не делает получение блокировки бесплатным по числу обменов — консенсусу всё ещё нужна коммуникация. И он не мешает конкурирующему процессу получить блокировку — напротив, получение новым держателем более высокого токена это нормальный, ожидаемый случай, вокруг которого и построен механизм."
  },
  "data-consensus-2": {
    "prompt": "Почему каждое решение консенсуса, например выбор лидера в Raft, требует обмена с большинством узлов, а не просто запроса к одному координатору?",
    "options": [
      "Чтобы согласованный факт пережил отказ меньшинства узлов — ответу единственного координатора нельзя доверять в одиночку, если сам координатор может ошибаться, оказаться отрезанным или отказавшим",
      "Потому что один координатор был бы так же безопасен, но быстрее, а обмен с большинством — лишь историческое соглашение",
      "Потому что протоколы консенсуса физически не способны учитывать голоса больше чем одного узла одновременно",
      "Потому что обмен с большинством устраняет необходимость для любого узла сохранять данные на диск"
    ],
    "explanation": "Требование большинства как раз и делает решение устойчивым к отказу любого меньшинства узлов, включая координатора, который сам может оказаться отрезанным сетью или неправым, — ответ единственного координатора такой гарантии не даёт. Это не просто соглашение: замена проверки большинства единственным координатором возвращает единую точку отказа, которую консенсус специально устраняет. Ничто в протоколах не мешает учитывать голоса многих узлов — это буквально то, что они делают. А долговечность/сохранение на диск — отдельный вопрос, не связанный с требованием кворума: узлам всё равно нужно сохранять состояние, чтобы пережить сбои."
  },
  "data-consensus-3": {
    "prompt": "Старый лидер зависает, избирается новый лидер с более высоким fencing token, и наконец до хранилища доходит запоздалая запись старого лидера. Что произойдёт и почему?",
    "code": "class InMemoryGuardedStorage implements GuardedStorage {\n  private lastToken = 0;\n  write(token: number, value: string): void {\n    if (token < this.lastToken) throw new Error('stale write rejected');\n    this.lastToken = token;\n  }\n}",
    "options": [
      "Хранилище отклонит её, потому что её токен меньше последнего токена, уже принятого хранилищем от нового лидера",
      "Хранилище молча примет её, перезаписав данные нового лидера, потому что записи применяются в порядке поступления",
      "Сервис блокировок задним числом отменит избрание нового лидера, чтобы пропустить старую запись",
      "Запись зависнет навсегда в ожидании, пока старый лидер повторно получит блокировку"
    ],
    "explanation": "Проверка сравнивает входящий токен с последним принятым и отклоняет всё, что старше, — именно это здесь и происходит: токен старого лидера меньше уже применённого токена нового лидера. Порядок поступления роли не играет — это как раз тот баг, для предотвращения которого и существуют fencing token. Решения консенсуса вроде завершённого выбора лидера не отменяются задним числом из-за запоздалой записи. А код бросает исключение немедленно, а не ждёт — никакого зависания здесь нет."
  },
  "data-idempotency-1": {
    "prompt": "Что именно позволяет серверу сделать ключ идемпотентности, когда один и тот же запрос приходит дважды?",
    "options": [
      "Распознать второй запрос как повтор первого и вернуть уже сохранённый результат вместо повторного выполнения побочного эффекта",
      "Зашифровать тело запроса, чтобы перехваченный запрос нельзя было повторно отправить злоумышленником",
      "Гарантировать, что сеть доставит запрос ровно один раз на транспортном уровне",
      "Автоматически откатить побочный эффект, выполненный первой попыткой"
    ],
    "explanation": "Единственная задача ключа идемпотентности — дать серверу отличить «эта операция уже выполнялась» от «это новая работа» и ответить соответственно; он не имеет отношения ни к шифрованию, ни к гарантиям доставки транспортного уровня, ни к автоматическому откату — ничего из этого ключ или его хранилище не предоставляют."
  },
  "data-idempotency-2": {
    "prompt": "Сервис платежей хранит под каждым ключом идемпотентности только HTTP-статус, а не тело ответа, чтобы хранилище было компактнее. Чем это приходится пожертвовать?",
    "options": [
      "Повтор с тем же ключом больше не может получить точно тот же ответ, который клиент увидел изначально, — воспроизводится только статус",
      "Ничем — одного статуса всегда достаточно, чтобы восстановить любой ответ, который может понадобиться клиенту",
      "Возможностью вообще обнаружить повтор, поскольку статусы не уникальны для каждого запроса",
      "Местом в хранилище — коды статусов на самом деле занимают больше места, чем типичное тело JSON-ответа"
    ],
    "explanation": "Хранится ровно то, что записано под ключом: если это только статус, повтору нечего вернуть, кроме статуса, — исходное тело ответа потеряно безвозвратно. Способность обнаруживать повтор определяется самим ключом, а не тем, что хранится под ним, так что второй вариант путает две разные вещи. Хранение статуса, а не полного тела, почти всегда компактнее, а не наоборот, так что четвёртый вариант тоже неверен."
  },
  "data-idempotency-3": {
    "prompt": "Согласно этому коду, что происходит при втором вызове store.run() с тем же ключом, если fn() из первого вызова уже завершился?",
    "code": "class IdempotencyStore {\n  private results = new Map<string, StoredResult>();\n\n  async run(key: string, fn: () => Promise<StoredResult>): Promise<StoredResult> {\n    const cached = this.results.get(key);\n    if (cached) return cached;\n\n    const result = await fn();\n    this.results.set(key, result);\n    return result;\n  }\n}",
    "options": [
      "fn() не вызывается повторно — возвращается результат, закэшированный после первого вызова",
      "fn() выполняется снова, и его новый результат молча перезаписывает закэшированный",
      "Хранилище выбрасывает исключение, потому что каждый ключ можно использовать только один раз",
      "Сохраняются оба результата, и вызывающий получает массив из двух ответов"
    ],
    "explanation": "Метод run() сначала проверяет this.results.get(key); если результат уже есть, он возвращается немедленно, и fn() не вызывается вовсе — именно так код избегает повторного выполнения побочного эффекта. Ни перезаписи, ни исключения, ни массива результатов код не производит."
  },
  "data-transactional-outbox-1": {
    "prompt": "Почему паттерн transactional outbox вставляет событие как строку в той же транзакции базы данных, что и бизнес-запись, вместо того чтобы публиковать в брокер напрямую из того же участка кода?",
    "options": [
      "Чтобы бизнес-запись и событие были атомарны друг относительно друга — сбой или простой брокера не могут оставить закоммиченным одно без другого",
      "Потому что брокеры сообщений не могут принимать записи из кода приложения, только из триггеров базы данных",
      "Чтобы ускорить запись, поскольку добавление строки в таблицу всегда быстрее сетевого вызова к брокеру",
      "Потому что SQL-базы данных требуют наличия строки outbox перед любым INSERT"
    ],
    "explanation": "Единственная причина — атомарность: одна локальная транзакция гарантирует, что бизнес-изменение и запись события либо оба закоммитятся, либо оба откатятся, закрывая именно то окно рассинхронизации, которое возникает при раздельных записи в БД и публикации в брокер. Брокеры прекрасно принимают вызовы из кода приложения — ограничение на триггеры БД выдумано. Дополнительная строка в транзакции не ускоряет запись, а немного её замедляет — просто цена того стоит. И SQL не требует никакой строки outbox перед INSERT — это требование самого паттерна, а не базы данных."
  },
  "data-transactional-outbox-2": {
    "prompt": "Команда переключает свой релей outbox с опроса раз в 5 секунд на считывание журнала репликации базы данных (коннектор change data capture). Что они выигрывают и какой ценой?",
    "options": [
      "Почти нулевую задержку публикации и отсутствие дополнительной нагрузки на чтение таблицы outbox — ценой эксплуатации инфраструктуры доступа к журналу/CDC вместо простого опроса",
      "Ничего не меняется — опрос и считывание журнала читают ровно те же данные с той же ценой",
      "Более строгие гарантии согласованности — шаг публикации релея становится доставкой ровно один раз вместо хотя бы один раз",
      "Возможность вообще не писать в таблицу outbox, поскольку журнал уже содержит событие"
    ],
    "explanation": "Считывание журнала репликации избавляет от интервала опроса (события публикуются почти сразу после коммита) и от повторяющихся SELECT по таблице outbox — но взамен требует настроить и эксплуатировать коннектор CDC, что не бесплатно. Природа данных, которые читает релей, не меняется — меняется лишь то, как быстро и как дёшево он их получает. Ни один из механизмов не превращает публикацию в доставку ровно один раз: шаг «опубликовать, затем пометить» остаётся at-least-once в обоих случаях. И таблица outbox по-прежнему нужна — именно её изменения читает журнал."
  },
  "data-transactional-outbox-3": {
    "prompt": "В этом релее — что произойдёт, если процесс упадёт сразу после успешного broker.publish(), но до выполнения UPDATE?",
    "code": "async function relayOnce(db: Db, broker: Broker) {\n  const rows = await db.query(\n    'SELECT * FROM outbox WHERE published_at IS NULL ORDER BY id LIMIT 100',\n  );\n  for (const row of rows) {\n    await broker.publish(row.type, row.payload);\n    await db.query('UPDATE outbox SET published_at = now() WHERE id = $1', [row.id]);\n  }\n}",
    "options": [
      "Строка будет опубликована повторно при следующем запуске, так как у неё всё ещё publishedAt = null, — событие доставляется хотя бы один раз, и потребитель обязан дедуплицировать",
      "Событие теряется навсегда, поскольку в базе данных нет записи о том, что его нужно опубликовать повторно",
      "Транзакция автоматически откатывает вызов broker.publish(), поэтому событие на самом деле никогда не доставляется",
      "Релей обнаруживает сбой и помечает строку опубликованной, не публикуя её повторно"
    ],
    "explanation": "Пометка published_at происходит отдельным запросом после публикации; если процесс падает между ними, строка остаётся с publishedAt = null и будет выбрана и опубликована заново при следующем запуске релея — то есть доставка гарантированно хотя бы один раз, а не ровно один раз, и дедупликация остаётся на стороне потребителя. broker.publish() — вызов к внешней системе, а не часть транзакции базы данных, так что откатить его невозможно. И ничего в этом коде не «обнаруживает сбой» и не помечает строку без публикации — после падения процесс просто перезапускается и читает те же неопубликованные строки заново."
  },
  "data-delivery-semantics-1": {
    "prompt": "Брокер настроен на доставку «ровно один раз», но потребитель время от времени получает одно и то же сообщение дважды. Что это на самом деле отражает?",
    "options": [
      "Ни один реальный транспорт не даёт настоящую доставку ровно один раз бесплатно — на самом деле обеспечивается доставка хотя бы один раз, и потребитель должен дедуплицировать, чтобы получить результат «фактически один раз»",
      "Брокер неправильно настроен, поскольку корректно настроенный брокер с exactly-once никогда не может доставить сообщение повторно",
      "Сообщение было повреждено при передаче, и контрольные суммы брокера это не поймали",
      "Потребитель по ошибке дважды подписан на один и тот же топик"
    ],
    "explanation": "Настройка «exactly-once» у брокера — это конфигурация транспорта, а не физический закон: реальные сбои (потребитель падает после применения эффекта, но до подтверждения; сеть задерживает подтверждение дольше таймаута) всё равно приводят к повторной доставке, и именно поэтому «эффективно один раз» достигается дедупликацией на стороне потребителя, а не одной лишь настройкой брокера. Остальные варианты выдумывают причины (повреждение данных, двойную подписку, неправильную конфигурацию), которых сам факт повтора не доказывает."
  },
  "data-delivery-semantics-2": {
    "prompt": "Команда переключает потребителя с подтверждения сообщения до применения его эффекта на подтверждение только после того, как эффект применён и закоммичен. Чем они жертвуют?",
    "options": [
      "Они меняют «никогда не доставляется повторно» на «никогда не теряется молча» — сбой в середине обработки теперь вызывает повторную доставку, с которой потребитель обязан справляться дедупликацией, вместо безвозвратной потери сообщения при сбое сразу после раннего подтверждения",
      "Наблюдаемо ничего не меняется, поскольку момент подтверждения не влияет на поведение доставки",
      "Они получают настоящую доставку ровно один раз, поскольку позднее подтверждение гарантирует, что брокер не сможет доставить сообщение повторно",
      "Они теряют возможность обрабатывать сообщения по порядку, поскольку позднее подтверждение отключает гарантии упорядочивания"
    ],
    "explanation": "Подтверждение до обработки рискует потерять сообщение навсегда, если сбой случится сразу после подтверждения, но до применения эффекта, — брокер решит, что всё в порядке, и не повторит доставку. Подтверждение после обработки убирает этот риск потери ценой того, что сбой в процессе обработки теперь приводит к повторной доставке, которую обязан обработать дедуплицирующий потребитель. Момент подтверждения — это именно то, что определяет это поведение, поэтому второй вариант неверен; ни истинного exactly-once, ни изменений в порядке доставки этот выбор сам по себе не даёт."
  },
  "data-delivery-semantics-3": {
    "prompt": "Этот обработчик применяет побочный эффект и записывает id сообщения в одной транзакции. Почему это важно?",
    "code": "async function handleMessage(db: Db, msg: { id: string; type: string; payload: unknown }) {\n  const already = await db.query('SELECT 1 FROM processed_messages WHERE id = $1', [msg.id]);\n  if (already.length > 0) return;\n\n  await db.transaction(async (tx) => {\n    await applySideEffect(tx, msg);\n    await tx.insert('processed_messages', { id: msg.id, processedAt: new Date() });\n  });\n}",
    "options": [
      "Это гарантирует, что сбой между двумя шагами невозможен, — эффект применяется и id записывается вместе или не происходит ни то ни другое, поэтому повторно доставленное сообщение либо полностью пропускается, либо полностью применяется и записывается заново, без рассинхронизации",
      "Это делает саму очередь сообщений транзакционной, поэтому брокер больше никогда не доставит это сообщение повторно",
      "Реальной пользы по сравнению с раздельной записью нет — транзакция здесь нужна только для стиля",
      "Это предотвращает обработку сообщения более одного раза на сетевом уровне"
    ],
    "explanation": "Без общей транзакции возможен ровно тот сценарий, который этот код закрывает: применить эффект, но упасть до записи id (повтор доставки применит эффект ещё раз) или записать id, но упасть до применения эффекта (повтор доставки молча пропустит эффект, который на самом деле не произошёл). Транзакция делает эти два шага атомарными друг относительно друга. Она не превращает саму очередь во что-то транзакционное и не действует на сетевом уровне — это исключительно механизм согласованности между эффектом и его записью внутри одной базы данных."
  },
  "data-optimistic-locking-1": {
    "prompt": "Два клиента читают одну и ту же строку (version=1), затем оба пытаются записать на основе этого чтения. Чем оптимистичная блокировка отличается от незащищённой записи?",
    "options": [
      "Каждая запись обусловлена прочитанной версией, поэтому условное обновление второго писателя не затрагивает ни одной строки и явно завершается неудачей вместо молчаливой перезаписи изменения первого писателя",
      "Она блокирует второму клиенту чтение строки, пока первый клиент не закончит запись",
      "Она автоматически сливает изменения обоих клиентов в одно объединённое обновление",
      "Она ставит обе записи в очередь и применяет их одну за другой в порядке поступления"
    ],
    "explanation": "Суть механизма — условие WHERE version = ожидаемая: если версия уже сдвинулась, обновление не находит подходящих строк и явно сообщает об этом вызывающей стороне, вместо того чтобы просто перезаписать чужое изменение. Оптимистичная блокировка ничего не блокирует на чтении, ничего не сливает автоматически и не превращает конкурентные записи в очередь — именно отсутствие блокировки и очереди и есть её смысл, в отличие от пессимистичной блокировки."
  },
  "data-optimistic-locking-2": {
    "prompt": "Команда добавляет оптимистичную блокировку к строке общего счётчика, получающей тысячи конкурентных инкрементов в секунду. Каков вероятный результат?",
    "options": [
      "Большинство обновлений проигрывают гонку за версию и вынуждены повторяться, поэтому пропускная способность может оказаться хуже, чем у пессимистичной блокировки или атомарного инкремента, которые просто сериализуют писателей вместо того, чтобы отклонять их",
      "Пропускная способность растёт, поскольку оптимистичная блокировка никогда не блокирует ни одного писателя",
      "Счётчик становится защищён от потерянных обновлений без каких-либо издержек, поскольку версионирование ничего не стоит при конкуренции",
      "Строка автоматически переключается на пессимистичную блокировку, как только конкуренция превышает порог"
    ],
    "explanation": "При тысячах конкурентных записей в секунду по одной строке почти каждая запись читает версию, которая успевает устареть ещё до попытки обновления, поэтому подавляющее большинство обновлений проигрывают гонку и должны повторяться — а повтор сам по себе снова читает и снова рискует устареть. Именно поэтому для горячих счётчиков обычно выбирают атомарный инкремент или пессимистичную блокировку вместо оптимистичной. Никакого автоматического переключения механизма блокировки в ответ на конкуренцию не существует, и версионирование определённо не бесплатно при высокой конкуренции."
  },
  "data-optimistic-locking-3": {
    "prompt": "В этом коде — при каком условии updateProfile выбрасывает ConflictError?",
    ...
    "options": [
      "Когда условие WHERE в UPDATE (id = ? AND version = ?) не соответствует ни одной строке, то есть какая-то другая запись уже продвинула версию дальше той, что прочитал этот вызывающий",
      "Каждый раз, когда в одном запросе обновляются два разных столбца",
      "Каждый раз, когда исчерпан пул соединений с базой данных",
      "Каждый раз, когда patch.name равен undefined"
    ],
    "explanation": "Код проверяет result.rowCount === 0 — единственный способ получить ноль затронутых строк здесь в том, что условие version = expectedVersion не совпало ни с одной строкой, то есть версия уже была продвинута другой записью. Количество обновляемых столбцов, состояние пула соединений и значение patch.name к этой проверке отношения не имеют."
  },
  "data-backpressure-1": {
    "prompt": "Сервис позволяет своей внутренней очереди работы расти без какого-либо ограничения размера во время всплеска трафика, вместо того чтобы её ограничить. Какое прямое последствие призван предотвратить backpressure?",
    "options": [
      "Неограниченный рост памяти, который в итоге приводит к падению процесса и утягивает с собой всю выполняемую работу, включая запросы, которые иначе завершились бы успешно, — вместо аккуратного отклонения избытка",
      "Очередь автоматически начинает обрабатывать элементы в неправильном порядке",
      "Потребитель навсегда перестаёт принимать любую новую работу, даже после окончания всплеска",
      "Уже полностью обработанные запросы молча выполняются повторно"
    ],
    "explanation": "Неограниченная очередь под быстрым продюсером растёт вместе со всплеском: память заканчивается, и процесс падает, забирая с собой все запросы, которые были в очереди или в обработке в этот момент, — именно этот сценарий и предотвращает ограничение очереди с явной политикой отказа. Ни порядок обработки, ни постоянная остановка приёма, ни повторное выполнение уже обработанных запросов не являются прямым следствием отсутствия ограничения — это несвязанные сценарии."
  },
  "data-backpressure-2": {
    "prompt": "Команда уменьшает вместимость ограниченной очереди с 10 000 до 100, чтобы отказывать быстрее при перегрузке. Чем они жертвуют ради этого более быстрого отказа?",
    "options": [
      "Они отклоняют бóльшую долю всплеска трафика сразу же — меньшая вместимость означает, что меньшая часть всплеска поглощается до того, как запросы начинают отклоняться",
      "Ничем — размер вместимости не влияет на то, какая доля всплеска принимается, только на задержку",
      "Они получают неограниченную пропускную способность, поскольку меньшая очередь всегда обрабатывает элементы быстрее",
      "Они исключают саму возможность нехватки памяти, независимо от размера элементов"
    ],
    "explanation": "Вместимость очереди — это именно тот буфер, который поглощает всплеск до того, как начнутся отказы; уменьшив его в сто раз, команда напрямую уменьшает долю всплеска, которую очередь способна принять, прежде чем начать отклонять запросы. Это прямо противоположно утверждению, что вместимость не влияет на объём принятого всплеска. Пропускная способность обработки определяется скоростью потребителя, а не размером буфера, а меньший буфер лишь снижает риск нехватки памяти, но не устраняет его полностью, если сам поток данных достаточно велик."
  },
  "data-backpressure-3": {
    "prompt": "В этом коде — что делает tryPush(), когда очередь уже содержит `capacity` элементов?",
    "code": "class BoundedQueue<T> {\n  private items: T[] = [];\n  constructor(private readonly capacity: number) {}\n\n  tryPush(item: T): boolean {\n    if (this.items.length >= this.capacity) {\n      return false;\n    }\n    this.items.push(item);\n    return true;\n  }\n}",
    "options": [
      "Возвращает false, не добавляя элемент, позволяя вызывающей стороне отклонить запрос вместо дальнейшего роста очереди",
      "Удаляет самый старый элемент, чтобы освободить место, а затем добавляет новый",
      "Блокируется, пока потребитель не вызовет pop() и не освободит место",
      "Выбрасывает исключение, которое приводит к падению процесса"
    ],
    "explanation": "Проверка if (this.items.length >= this.capacity) return false; выполняется до какого-либо изменения массива items, поэтому при заполненной очереди метод просто возвращает false и ничего не добавляет и не удаляет — сигнал backpressure, а не вытеснение, блокировка или исключение. Остальные варианты описывают другие, вполне реальные стратегии переполнения очереди, но не ту, что реализована именно в этом коде."
  },
  "data-change-data-capture-1": {
    "prompt": "Почему CDC-коннектор читает журнал упреждающей записи/репликации базы данных вместо периодических запросов к таблицам в поисках изменений?",
    "options": [
      "Опрос может пропускать промежуточные состояния между опросами и подвержен гонкам с конкурентными записями, тогда как журнал фиксирует каждое закоммиченное изменение ровно один раз, в истинном порядке коммитов",
      "Запрашивать таблицы технически невозможно, пока к базе данных подключён CDC-коннектор",
      "Журнал репликации — единственное место, где вообще хранятся данные строк, поэтому таблицы нельзя запрашивать напрямую",
      "Чтение из журнала всегда быстрее любого SQL-запроса независимо от размера таблицы"
    ],
    "explanation": "Ключевая проблема опроса — не скорость, а корректность: между двумя опросами строка может измениться несколько раз, и опрос увидит только финальное состояние, а конкурентная запись, попавшая в окно опроса, может быть пропущена или посчитана дважды. Журнал же фиксирует каждое изменение по отдельности, в порядке коммита. Таблицы остаются полностью доступными для запросов даже при подключённом CDC-коннекторе, и журнал не единственное место хранения данных строк — это отдельная от табличных данных структура."
  },
  "data-change-data-capture-2": {
    "prompt": "Команда заменяет CDC на основе журнала ночной задачей снапшот-и-сравнение, чтобы упростить пайплайн. Чем они жертвуют?",
    "options": [
      "Видимостью промежуточных состояний — строка, обновлённая дважды между снапшотами, покажет только своё финальное значение, а история изменений между ними будет потеряна",
      "Ничем существенным — снапшот-и-сравнение фиксирует ровно ту же информацию, что и коннектор на основе журнала, просто реже",
      "Возможностью вообще обнаружить, что строка была удалена",
      "Нагрузкой на чтение исходной базы данных, которую CDC на основе журнала всегда добавляет, а снапшоты — никогда"
    ],
    "explanation": "Снапшот-и-сравнение сравнивает состояние на момент каждого снимка, поэтому любые промежуточные изменения между снимками — включая случаи, когда строка обновлялась несколько раз или была создана и удалена в промежутке, — становятся невидимыми; это качественно меньше информации, а не то же самое реже. Обнаружить удаление всё ещё можно, сравнив наборы ключей между снимками. А нагрузку на чтение источника добавляет именно периодический снапшот (полное или частичное сканирование таблиц), тогда как CDC на основе журнала как раз минимизирует эту нагрузку, читая уже существующий журнал репликации."
  },
  "data-change-data-capture-3": {
    "prompt": "В этом CdcConsumer — что даёт проверка `event.lsn <= this.lastLsn` перед применением события?",
    "code": "class CdcConsumer {\n  private lastLsn = 0n;\n\n  async apply(event: ChangeEvent) {\n    if (event.lsn <= this.lastLsn) return;\n    await this.project(event);\n    this.lastLsn = event.lsn;\n  }\n}",
    "options": [
      "Делает повторную доставку уже применённого события безопасным no-op, поскольку LSN только возрастает, и LSN меньше или равный последнему означает, что именно это изменение уже обработано",
      "Сортирует входящие события по возрастанию LSN перед их применением",
      "Блокирует потребителю обработку операций insert, оставляя только update и delete",
      "Гарантирует, что исходная база данных никогда не отстанет в репликации"
    ],
    "explanation": "Проверка сравнивает LSN нового события с последним успешно применённым: поскольку LSN монотонно возрастает вместе с порядком коммитов, значение меньше или равное this.lastLsn однозначно означает уже обработанное изменение, и apply() просто выходит, ничего не делая, — это и есть дедупликация при повторной доставке. Сама проверка не переупорядочивает события (для этого нужна была бы буферизация и сортировка отдельно), не различает типы операций и никак не влияет на реальную задержку репликации исходной базы — это забота инфраструктуры, а не потребителя."
  }
};

  ...
  if (!q) {
    ...
    return (
      <div className="text-center py-12 space-y-4">
        ...
        <button onClick={restart} className="rounded-lg bg-accent px-4 py-2 font-medium text-white hover:bg-accent-strong">Пройти заново</button>
      </div>
    );
  }
      ...
      {selected !== null && (
        <div className="rounded-lg bg-surface-raised border border-surface-muted p-4">
          ...
          <div className="mt-3 flex justify-end">
            <button onClick={nextQuestion} className="rounded-lg bg-accent px-4 py-2 font-medium text-white hover:bg-accent-strong">Дальше →</button>
          </div>
        </div>
      )}
    </div>
